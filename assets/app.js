...
function mgmtPct(cur,prev){return prev>0?(cur-prev)/prev*100:(cur>0?100:0)}
...
function mgmtBuild(){const occupied=faccoes.filter(f=>f.status==='ATIVA'&&String(f.faccao||'').trim());const base=occupied.map(f=>{const w=mgmtPeriodStats(f.group,7),wp=mgmtPeriodStats(f.group,7,7),m=mgmtPeriodStats(f.group,30),mp=mgmtPeriodStats(f.group,30,30);return {f,w,m,wTrend:mgmtPct(w.avg,wp.avg),mTrend:mgmtPct(m.avg,mp.avg)}});const medBySeg={};[...new Set(base.map(x=>x.f.segmento||'OUTROS'))].forEach(seg=>{medBySeg[seg]=mgmtMedian(base.filter(x=>(x.f.segmento||'OUTROS')===seg&&x.w.days>=2).map(x=>x.w.avg))});return base.map(x=>{const med=medBySeg[x.f.segmento||'OUTROS']||mgmtMedian(base.map(y=>y.w.avg));let status='OK',reasons=[];if(x.w.days<2&&x.m.days<3){status='SEM_DADOS';reasons.push('dados insuficientes para avaliar')}else{const rel=med?x.w.avg/med:1;if(x.w.days<3||x.w.coverage<60){status='SEM_DADOS';reasons.push('coleta semanal incompleta')}else if((x.wTrend<=-30&&x.mTrend<=-15)||(rel<.55&&x.wTrend<-10)){status='CRITICA';reasons.push(`queda semanal de ${Math.abs(x.wTrend).toFixed(0)}%`);if(rel<.55)reasons.push('média muito abaixo do segmento')}else if(x.wTrend<=-15||rel<.75||x.w.coverage<80){status='ATENCAO';if(x.wTrend<=-15)reasons.push(`queda semanal de ${Math.abs(x.wTrend).toFixed(0)}%`);if(rel<.75)reasons.push('abaixo da referência do segmento');if(x.w.coverage<80)reasons.push('coletas parcialmente incompletas')}else{if(x.wTrend>=15)reasons.push(`crescimento semanal de ${x.wTrend.toFixed(0)}%`);else reasons.push('desempenho estável')}}let score=status==='CRITICA'?100:status==='ATENCAO'?60:status==='SEM_DADOS'?20:0;score+=Math.max(0,-x.wTrend);return {...x,status,reasons,score,segmentMedian:med}})}
function mgmtLabel(s){return s==='OK'?'SAUDÁVEL':s==='ATENCAO'?'ATENÇÃO':s==='CRITICA'?'CRÍTICA':'DADOS INSUFICIENTES'}
function mgmtAction(x){if(x.status==='CRITICA')return 'Conversar com a liderança e revisar contingente/atividade.';if(x.status==='ATENCAO')return 'Acompanhar de perto e verificar com a liderança se houve situação específica.';if(x.status==='SEM_DADOS')return 'Conferir as coletas antes de avaliar a organização.';if(x.wTrend>=15)return 'Reconhecer a evolução e agradecer a liderança.';return 'Manter acompanhamento normal.'}
function mgmtMessage(x){const name=x.f.faccao||x.f.group;if(x.status==='OK')return `Boa noite, liderança da ${name}.\n\nPassando para agradecer pelo trabalho que vocês vêm realizando. As métricas da organização apresentam ${x.wTrend>=15?'uma evolução positiva':'boa estabilidade'} nesta semana, com média de ${x.w.avg.toFixed(1)} de contingente nas coletas.\n\nContinuem mantendo a organização e o contingente nesse ritmo. Parabéns pelo trabalho e contem com a equipe do Ilegal.`;if(x.status==='SEM_DADOS')return `Boa noite, liderança da ${name}.\n\nEstamos revisando o acompanhamento das organizações e identificamos que ainda não temos coletas suficientes desta semana para uma avaliação segura da ${x.f.group}.\n\nEstamos conferindo os dados antes de qualquer análise. Caso tenha ocorrido alguma situação específica, podem nos sinalizar.`;return `Boa noite, liderança da ${name}.\n\nEstamos acompanhando as métricas da ${x.f.group} e identificamos ${x.reasons.join(' e ')}. A média semanal atual está em ${x.w.avg.toFixed(1)}, com variação de ${x.wTrend>=0?'+':''}${x.wTrend.toFixed(0)}% em relação à semana anterior.\n\nQueremos entender se aconteceu alguma situação específica nos últimos dias e alinhar com vocês o que podemos fazer para recuperar os números. Quando puderem, chamem a equipe do Ilegal para conversarmos.`}
