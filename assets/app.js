...
let dashboardConfig={...DEFAULT_DASHBOARD_CONFIG},dashboardAlertStates=[],spotifyConfig={url:'',clientId:''},chatUnsubscribe=null,chatItems=[],chatPendingAttachment=null,spotifyPlayer=null,spotifyDeviceId='',spotifyAccessToken='',spotifyTokenExpiry=0;
...
function metricDailyCard(day,title){const hours=['14H','16H','21H','23H'],vals=hours.filter(h=>Number.isFinite(day?.slots?.[h])).map(h=>day.slots[h]);const peak=vals.length?Math.max(...vals):null,avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:null;return `<section class="metric-daily-card"><header><div><span>${esc(title)}</span><h3>${day?esc(day.date.toLocaleDateString('pt-BR')):'SEM COLETA'}</h3></div><div><small>PICO</small><b>${peak??'—'}</b></div><div><small>MÉDIA</small><b>${avg===null?'—':avg.toFixed(1)}</b></div></header><div class="metric-hour-grid">${hours.map(h=>`<div class="metric-hour-cell ${day&&Number.isFinite(day.slots[h])?'collected':'waiting'}"><span>${h.replace('H',':00')}</span><b>${day&&Number.isFinite(day.slots[h])?day.slots[h]:'—'}</b><small>${day&&Number.isFinite(day.slots[h])?'ONLINE':'NÃO COLETADO'}</small></div>`).join('')}</div></section>`}
...
// ===== HIGH OS V8.25 · COMUNICAÇÃO FLUTUANTE + SPOTIFY CONNECT =====
...
function chatAttachmentHtml(m){const a=m.anexo;if(!a)return '';if(String(a.type||'').startsWith('image/'))return `<a class="chat-attachment image" href="${esc(a.dataUrl)}" target="_blank"><img src="${esc(a.dataUrl)}" alt="${esc(a.name||'imagem')}"><span>${esc(a.name||'imagem')}</span></a>`;return `<a class="chat-attachment file" href="${esc(a.dataUrl)}" download="${esc(a.name||'arquivo')}">📎 ${esc(a.name||'arquivo')} <small>${Math.round((a.size||0)/1024)} KB</small></a>`}
function chatMeetingHtml(m){return m.reuniao?.url?`<button type="button" class="chat-meeting-invite" data-meeting-url="${esc(m.reuniao.url)}" data-room="${esc(m.reuniao.room||'Sala')}">🎙 ENTRAR NA REUNIÃO <small>${esc(m.reuniao.room||'')}</small></button>`:''}
function renderChatMessages(items=[]){chatItems=items;const me=(currentUser?.email||'').toLowerCase();const body=items.length?items.map(m=>`<article class="chat-message ${String(m.email||'').toLowerCase()===me?'mine':''}"><img src="${esc(m.photoURL||'')}" alt="" onerror="this.style.display='none'"><div><header><b>${esc(m.nome||m.email||'Usuário')}</b><small>${esc(m.cargo||'')} • ${esc(chatTime(m.createdAt||m))}</small></header>${m.texto?`<p>${esc(m.texto)}</p>`:''}${chatAttachmentHtml(m)}${chatMeetingHtml(m)}${isAdmin()?`<button type="button" class="chat-delete" data-chat-delete="${esc(m.id)}" title="Excluir mensagem">×</button>`:''}</div></article>`).join(''):'<div class="chat-empty">Nenhuma mensagem ainda. Inicie a conversa com a equipe.</div>';['#chatMessages','#floatingChatMessages'].forEach(sel=>{const box=$(sel);if(!box)return;box.innerHTML=body;box.scrollTop=box.scrollHeight;box.querySelectorAll('[data-chat-delete]').forEach(b=>b.onclick=()=>deleteChatMessage(b.dataset.chatDelete));box.querySelectorAll('[data-meeting-url]').forEach(b=>b.onclick=()=>openTeamMeeting(b.dataset.meetingUrl,b.dataset.room))})}
function startChat(){if(chatUnsubscribe||!currentUser||!canViewModule('chat'))return;const launch=$('#teamChatLauncher');if(launch)launch.classList.remove('hidden');try{chatUnsubscribe=onSnapshot(chatCol,qs=>{const items=qs.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>{const ta=a.createdAt?.seconds||new Date(a.createdAtText||0).getTime()/1000,tb=b.createdAt?.seconds||new Date(b.createdAtText||0).getTime()/1000;return ta-tb}).slice(-150);renderChatMessages(items)},e=>{['#chatMessages','#floatingChatMessages'].forEach(s=>{const box=$(s);if(box)box.innerHTML=`<div class="chat-empty">Não foi possível carregar o chat: ${esc(e.message)}</div>`})})}catch(e){console.warn(e)}}
async function sendChatMessage(inputSelector='#floatingChatInput'){if(!canEditModule('chat'))return permissionDeniedMessage('chat',true);const input=$(inputSelector),texto=input?.value.trim()||'';if(!texto&&!chatPendingAttachment)return;if(texto.length>1000)return alert('Mensagem muito longa. Limite: 1000 caracteres.');try{await addDoc(chatCol,{texto,anexo:chatPendingAttachment||null,email:currentUser.email||'',nome:currentProfile?.name||currentUser.displayName||currentUser.email,cargo:currentProfile?.cargo||currentProfile?.role||'',photoURL:currentUser.photoURL||'',sessionId:currentSessionId||'',createdAt:serverTimestamp(),createdAtText:new Date().toISOString()});if(input)input.value='';chatPendingAttachment=null;renderChatAttachmentPreview()}catch(e){alert('Erro ao enviar mensagem: '+e.message)}}
...
function toggleFloatingChat(force){const p=$('#teamChatFloat');if(!p)return;const show=force===undefined?p.classList.contains('hidden'):!!force;p.classList.toggle('hidden',!show);if(show)setTimeout(()=>$('#floatingChatInput')?.focus(),50)}
...
async function startTeamMeeting(){if(!canEditModule('chat'))return permissionDeniedMessage('chat',true);const room=`high-os-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,url=`https://meet.jit.si/${room}`;await addDoc(chatCol,{texto:'Reunião iniciada no High OS',reuniao:{room,url},email:currentUser.email||'',nome:currentProfile?.name||currentUser.displayName||currentUser.email,cargo:currentProfile?.cargo||currentProfile?.role||'',photoURL:currentUser.photoURL||'',createdAt:serverTimestamp(),createdAtText:new Date().toISOString()});openTeamMeeting(url,room)}
function openTeamMeeting(url,room='Sala High OS'){const o=$('#teamMeetingOverlay'),f=$('#teamMeetingFrame');if(!o||!f)return;$('#teamMeetingRoomLabel').textContent=`${room} • áudio • vídeo • compartilhar tela`;f.src=url+'#config.prejoinConfig.enabled=false&config.startWithAudioMuted=true';o.classList.remove('hidden')}
function closeTeamMeeting(){const o=$('#teamMeetingOverlay'),f=$('#teamMeetingFrame');if(f)f.src='about:blank';o?.classList.add('hidden')}

$('#spotifySaveBtn')?.addEventListener('click',saveSpotifyConfig);$('#spotifySaveClientBtn')?.addEventListener('click',saveSpotifyClient);$('#spotifyLoginBtn')?.addEventListener('click',spotifyLogin);$('#spotifyDockOpenPage')?.addEventListener('click',()=>activateAppPage('spotify'));$('#spotifySearchBtn')?.addEventListener('click',spotifySearch);$('#spotifySearchInput')?.addEventListener('keydown',e=>{if(e.key==='Enter')spotifySearch()});$('#spotifyPlayBtn')?.addEventListener('click',()=>spotifyPlayer?.togglePlay());$('#spotifyPrevBtn')?.addEventListener('click',()=>spotifyPlayer?.previousTrack());$('#spotifyNextBtn')?.addEventListener('click',()=>spotifyPlayer?.nextTrack());$('#spotifyDockMain')?.addEventListener('click',e=>{if(e.target.closest('#spotifyPlayGlyph'))spotifyPlayer?.togglePlay()});
$('#teamChatLauncher')?.addEventListener('click',()=>toggleFloatingChat());$('#chatFloatMin')?.addEventListener('click',()=>toggleFloatingChat(false));$('#chatPageOpenFloat')?.addEventListener('click',()=>toggleFloatingChat(true));$('#floatingChatSendBtn')?.addEventListener('click',()=>sendChatMessage('#floatingChatInput'));$('#floatingChatInput')?.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendChatMessage('#floatingChatInput')}});$('#chatAttachInput')?.addEventListener('change',e=>{prepareChatAttachment(e.target.files?.[0]);e.target.value=''});$('#chatMeetingBtn')?.addEventListener('click',startTeamMeeting);$('#teamMeetingClose')?.addEventListener('click',closeTeamMeeting);document.querySelectorAll('[data-chat-emoji]').forEach(b=>b.addEventListener('click',()=>{const i=$('#floatingChatInput');if(i){i.value+=b.dataset.chatEmoji;i.focus()}}));
...
console.info('HIGH OS V8.25 · Métricas Intelligence + Chat flutuante + Reuniões + Spotify Connect');
