...
function jitsiUrl(room){const display=encodeURIComponent(hmDisplayName());return `https://meet.jit.si/${room}#userInfo.displayName=${display}&config.prejoinConfig.enabled=false&config.startWithAudioMuted=false&config.startWithVideoMuted=true&config.disableDeepLinking=true&config.enableWelcomePage=false`}
async function startTeamMeeting(mode='video'){if(!canEditModule('chat'))return permissionDeniedMessage('chat',true);if(!chatRecipientEmail)return alert('Selecione o usuário que deseja chamar.');const room=`high-os-dm-${Date.now()}-${Math.random().toString(36).slice(2,8)}`,url=`https://meet.jit.si/${room}`;try{await addDoc(chatCol,{texto:'',reuniao:{room,url,status:'calling',mode},recipientEmail:chatRecipientEmail,conversationId:chatConversationId(currentUser.email,chatRecipientEmail),email:currentUser.email||'',nome:currentProfile?.name||currentUser.displayName||currentUser.email,cargo:currentProfile?.cargo||currentProfile?.role||'',photoURL:currentProfile?.photoURL||currentUser.photoURL||'',createdAt:serverTimestamp(),createdAtText:new Date().toISOString()});const target=hmUser(chatRecipientEmail);openTeamMeeting(url,room,'',`Chamada • ${target?hmUserName(target):'Equipe'}`,mode==='audio')}catch(e){alert('Não foi possível iniciar a chamada: '+e.message)}}
...
function joinTeamVoiceChannel(id){if(!canViewModule('chat'))return permissionDeniedMessage('chat');const c=HM_VOICE_CHANNELS[id];if(!c)return;openTeamMeeting(`https://meet.jit.si/${c.room}`,c.room,id,c.name,false)}
...
console.info('HIGH OS V8.28 · High Messenger + DMs persistentes + canais de voz em equipe');
