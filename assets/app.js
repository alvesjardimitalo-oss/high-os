...
window.gsCopyRowCds=function(i){const r=gsRows()[i];navigator.clipboard?.writeText([r?.cds,r?.secondary].filter(Boolean).join('\n'))}
...
function v9CleanRows(rows){return (rows||[]).map(r=>({tipo:r.tipo||'OUTRO',nome:r.nome||'',cds:r.cds||'',secondary:r.secondary||'',radio:r.radio||''}));}
...
function v9StructureRequest(group,diff){const lines=[`Assunto: Alterações estruturais - ${group}`,'','Solicitação:','',`- Favor realizar as seguintes alterações no Group "${group}":`,''];diff.forEach((d,i)=>{const r=d.depois||d.antes;lines.push(`${i+1}. ${d.acao} ${r.tipo} - ${r.nome}`);if(d.acao==='ALTERAR'){lines.push(`- Atual: ${d.antes.cds||'—'}${d.antes.secondary?` | Spawn: ${d.antes.secondary}`:''}`);lines.push(`- Novo: ${d.depois.cds||'—'}${d.depois.secondary?` | Spawn: ${d.depois.secondary}`:''}`)}else{if(r.cds)lines.push(`- CDS: ${r.cds}`);if(r.secondary)lines.push(`- Spawn/CDS secundária: ${r.secondary}`);if(r.tipo==='RÁDIO'&&(r.radio||r.nome))lines.push(`- Rádio: ${r.radio||r.nome}`)}lines.push('')});lines.push(`- Permissão / Group: "${group}"`);return lines.join('\n')}
...
function v9EditorHtml(row={},isNew=false){return `<div class="gs-modal-backdrop" id="gsModal"><div class="gs-modal"><div class="gs-modal-head"><div><div class="eyebrow">${isNew?'ADICIONAR':'EDITAR'} ESTRUTURA</div><h3>${esc(row.nome||'Estrutura')}</h3></div><button class="mini-btn" id="gsmClose">✕</button></div><div class="gs-modal-grid"><label>TIPO<select id="gsmType">${GS_TYPES.map(t=>`<option ${row.tipo===t?'selected':''}>${t}</option>`).join('')}</select></label><label>NOME / IDENTIFICAÇÃO<input id="gsmName" value="${esc(row.nome||'')}"></label><label>CDS PRINCIPAL / BLIP<input id="gsmCds" value="${esc(row.cds||'')}" placeholder="x,y,z,h"></label><label>SPAWN / CDS SECUNDÁRIA<input id="gsmSecondary" value="${esc(row.secondary||'')}" placeholder="x,y,z,h"></label><label id="v9RadioField" class="${row.tipo==='RÁDIO'?'':'hidden'}">FREQUÊNCIA<input id="gsmRadio" value="${esc(row.radio||((row.tipo==='RÁDIO'&&/^\d+(\.\d+)?$/.test(row.nome||''))?row.nome:''))}" placeholder="Ex.: 123"></label></div><div class="gs-modal-actions"><button class="btn-secondary compact" id="gsmCancel">CANCELAR</button><button class="btn-primary compact" id="gsmSave">${isNew?'ADICIONAR':'GRAVAR EDIÇÃO'}</button></div></div></div>`}
gsOpenEditor=function(i=-1){const isNew=i<0,row=isNew?{tipo:'OUTRO',nome:'',cds:'',secondary:'',radio:''}:{...gsRows()[i]};document.body.insertAdjacentHTML('beforeend',v9EditorHtml(row,isNew));const close=()=>$('#gsModal')?.remove();$('#gsmClose').onclick=close;$('#gsmCancel').onclick=close;$('#gsmType').onchange=e=>$('#v9RadioField')?.classList.toggle('hidden',e.target.value!=='RÁDIO');$('#gsmSave').onclick=()=>{const n={tipo:$('#gsmType').value,nome:$('#gsmName').value.trim(),cds:$('#gsmCds').value.trim(),secondary:$('#gsmSecondary').value.trim(),radio:$('#gsmRadio')?.value.trim()||''};if(!n.nome)return alert('Informe o nome/identificação.');if(isNew)gsRows().push(n);else gsRows()[i]=n;v9MarkDirty();close();gsRender(false)}};
...
console.info('HIGH OS V9.0 · Estrutura simples: uma base, mapa, edição em lote e solicitação opcional ao final.');
