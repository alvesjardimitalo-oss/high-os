...
// ===== HIGH OS V7.15 · CENTRAL RH EXPERIENCE · INTERVALO LIVRE + PRESENÇA =====
let metricFilterMode='month',metricRangeFrom='',metricRangeTo='';
function metricIsoDate(d){if(!d||isNaN(d))return '';return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
function metricInputDate(v=''){if(!v)return null;const m=String(v).match(/^(\d{4})-(\d{2})-(\d{2})$/);return m?new Date(+m[1],+m[2]-1,+m[3]):null}
function metricAllDateBounds(){const ds=metricas.map(metricDateValue).filter(d=>d&&d.getTime()).sort((a,b)=>a-b);return ds.length?{min:ds[0],max:ds.at(-1)}:{min:null,max:null}}
function metricActiveLabel(){if(metricFilterMode==='range'){const a=metricInputDate(metricRangeFrom),b=metricInputDate(metricRangeTo);return a&&b?`${a.toLocaleDateString('pt-BR')} → ${b.toLocaleDateString('pt-BR')}`:'INTERVALO PERSONALIZADO'}return metricPeriodLabel(metricPeriodKey)}
function metricActiveShortLabel(){return metricFilterMode==='range'?metricActiveLabel():metricPeriodLabel(metricPeriodKey)}
function metricRangeFilterRows(){const from=metricInputDate(metricRangeFrom),to=metricInputDate(metricRangeTo);if(!from||!to)return [];const a=new Date(from.getFullYear(),from.getMonth(),from.getDate()),b=new Date(to.getFullYear(),to.getMonth(),to.getDate(),23,59,59,999);return metricas.filter(m=>{const d=metricDateValue(m);return d&&d>=a&&d<=b})}
activeMetricRows=function(){if(metricFilterMode==='range')return metricRangeFilterRows();const key=metricPeriodKey||currentMetricMonthKey();return metricas.filter(m=>metricMonthKey(m)===key)};
metricGroupRecords=function(group){const key=alvesNorm(String(group||'')).replace(/\s+/g,'');return activeMetricRows().filter(m=>alvesNorm(String(m.group||m.organizacao||m.faccao||'')).replace(/\s+/g,'')===key).sort((a,b)=>metricDateValue(a)-metricDateValue(b))};

function metricPresenceStats(group){
 const x=metricAdvancedStats(group);if(!x)return null;const values=x.rows.flatMap(r=>Object.values(metricSlots(r)));const positives=values.filter(v=>v>0).length;const activePct=values.length?positives/values.length*100:0;const zeroCount=values.filter(v=>v===0).length;const dayRows=x.rows.map(r=>({r,avg:metricDayAverage(r)}));const best=dayRows.slice().sort((a,b)=>b.avg-a.avg)[0];const weak=dayRows.slice().sort((a,b)=>a.avg-b.avg)[0];const positiveDays=dayRows.filter(d=>d.avg>0).length;const dayPresence=dayRows.length?positiveDays/dayRows.length*100:0;const strongest=Object.entries(x.hourAvg).sort((a,b)=>b[1]-a[1])[0]||['—',0];const weakest=Object.entries(x.hourAvg).sort((a,b)=>a[1]-b[1])[0]||['—',0];return {...x,activePct,zeroCount,positiveDays,dayPresence,best,weak,strongest,weakest,values};
}
function metricTrendPill(v){const cls=v>5?'up':v<-5?'down':'neutral',txt=v>0?`+${v.toFixed(0)}%`:v<0?`${v.toFixed(0)}%`:'ESTÁVEL';return `<span class="rh-pill ${cls}">${txt}</span>`}
function metricSpark(rows=[]){const vals=rows.map(metricDayAverage),mx=Math.max(1,...vals);return `<div class="rh-spark">${vals.map(v=>`<i style="height:${Math.max(4,v/mx*34)}px" title="${v.toFixed(1)}"></i>`).join('')}</div>`}
function renderMetricPresence(){
 const grid=$('#metricPresenceGrid'),sum=$('#metricPresenceSummary');if(!grid||!sum)return;const seg=$('#metricSegment')?.value||'',q=alvesNorm($('#metricSearch')?.value||''),mode=$('#metricPresenceSort')?.value||'avg';let rows=metricSummaryRows().filter(x=>(!seg||x.f.segmento===seg)&&(!q||alvesNorm([x.f.group,x.f.faccao,x.f.qg].join(' ')).includes(q))).map(r=>({...r,p:metricPresenceStats(r.f.group)})).filter(r=>r.p);
 rows.sort((a,b)=>mode==='active'?b.p.activePct-a.p.activePct:mode==='peak'?b.p.peak.value-a.p.peak.value:mode==='trend'?b.p.trend-a.p.trend:mode==='low'?a.p.avg-b.p.avg:b.p.avg-a.p.avg);
 const avg=rows.length?rows.reduce((n,r)=>n+r.p.avg,0)/rows.length:0,active=rows.length?rows.reduce((n,r)=>n+r.p.activePct,0)/rows.length:0,zeros=rows.reduce((n,r)=>n+r.p.zeroCount,0),up=rows.filter(r=>r.p.trend>=15).length;
 sum.innerHTML=`<div class="rh-card feature"><span>FACÇÕES NO RECORTE</span><b>${rows.length}</b><small>${esc(metricActiveLabel())}</small></div><div class="rh-card"><span>CONTINGENTE MÉDIO</span><b>${avg.toFixed(1)}</b><small>média entre as organizações exibidas</small></div><div class="rh-card"><span>PRESENÇA ATIVA MÉDIA</span><b>${active.toFixed(0)}%</b><small>coletas com contingente acima de zero</small></div><div class="rh-card"><span>EM CRESCIMENTO</span><b>${up}</b><small>tendência recente ≥ 15%</small></div>`;
 grid.innerHTML=rows.length?rows.map(({f,p})=>`<article class="rh-presence-card" data-open-group="${esc(f.group)}"><header><div><h4>${esc(f.faccao||f.group)}</h4><small>${esc(f.group)} • ${esc(f.segmento||'—')}</small></div><div><div class="rh-presence-score">${p.avg.toFixed(1)}</div>${metricTrendPill(p.trend)}</div></header><div class="rh-presence-kpis"><div><span>PRESENÇA ATIVA</span><b>${p.activePct.toFixed(0)}%</b></div><div><span>PICO</span><b>${p.peak.value}</b></div><div><span>MELHOR HORÁRIO</span><b>${esc(p.strongest[0])}</b></div></div><div class="rh-hour-strip">${Object.entries(p.hourAvg).map(([h,v])=>`<div><span>${h}</span><b>${v.toFixed(1)}</b></div>`).join('')}</div>${metricSpark(p.rows)}<small style="opacity:.55">Clique para abrir a ficha completa • ${p.rows.length} dia(s)</small></article>`).join(''):'<div class="placeholder"><h3>SEM DADOS NO RECORTE</h3></div>';
}
function renderMetricRadar(rows=[]){
 const box=$('#metricRadar');if(!box)return;const stats=rows.map(r=>({...r,p:metricPresenceStats(r.f.group)})).filter(r=>r.p);const alerts=[];
 const falling=stats.slice().sort((a,b)=>a.p.trend-b.p.trend).find(x=>x.p.trend<=-15);if(falling)alerts.push({icon:'↓',title:`${falling.f.faccao||falling.f.group} em queda`,sub:`${falling.f.group} • últimos dias vs. período anterior`,val:`${falling.p.trend.toFixed(0)}%`});
 const growing=stats.slice().sort((a,b)=>b.p.trend-a.p.trend).find(x=>x.p.trend>=15);if(growing)alerts.push({icon:'↑',title:`${growing.f.faccao||growing.f.group} em crescimento`,sub:`${growing.f.group} • tendência recente`,val:`+${growing.p.trend.toFixed(0)}%`});
 const absent=stats.slice().sort((a,b)=>a.p.activePct-b.p.activePct)[0];if(absent)alerts.push({icon:'◌',title:'Menor presença ativa',sub:`${absent.f.faccao||absent.f.group} • ${absent.f.group}`,val:`${absent.p.activePct.toFixed(0)}%`});
 const peak=stats.slice().sort((a,b)=>b.p.peak.value-a.p.peak.value)[0];if(peak)alerts.push({icon:'◆',title:'Maior contingente registrado',sub:`${peak.f.faccao||peak.f.group} • ${peak.p.peak.hour} em ${peak.p.peak.date}`,val:String(peak.p.peak.value)});
 box.innerHTML=alerts.length?alerts.slice(0,4).map(a=>`<div class="rh-radar-item"><div class="rh-radar-icon">${a.icon}</div><div><strong>${esc(a.title)}</strong><small>${esc(a.sub)}</small></div><div class="rh-radar-value">${esc(a.val)}</div></div>`).join(''):'<div class="muted">Sem alertas estatísticos relevantes neste recorte.</div>';
}
function renderMetricDistribution(raw=[]){
 const box=$('#metricDistribution');if(!box)return;const vals=raw.flatMap(r=>Object.values(metricSlots(r))),bands=[['0',v=>v===0],['1–5',v=>v>=1&&v<=5],['6–10',v=>v>=6&&v<=10],['11–15',v=>v>=11&&v<=15],['16+',v=>v>=16]],counts=bands.map(([n,fn])=>[n,vals.filter(fn).length]),mx=Math.max(1,...counts.map(x=>x[1]));box.innerHTML=counts.map(([n,c])=>`<div class="rh-dist-row"><span>${n} online</span><div class="rh-dist-track"><div class="rh-dist-fill" style="width:${c/mx*100}%"></div></div><b>${c}</b></div>`).join('')+`<small style="opacity:.55">${vals.length} coletas horárias analisadas.</small>`;
}
renderMetrics=function(err=null){
 if(err instanceof Event)err=null;const q=alvesNorm($('#metricSearch')?.value||''),seg=$('#metricSegment')?.value||'';let rows=metricSummaryRows().filter(x=>(!seg||x.f.segmento===seg)&&(!q||alvesNorm([x.f.group,x.f.faccao,x.f.qg].join(' ')).includes(q)));const active=activeMetricRows(),groupKeys=new Set(rows.map(x=>alvesNorm(x.f.group).replace(/\s+/g,''))),visibleRaw=active.filter(m=>groupKeys.has(alvesNorm(String(m.group||m.organizacao||m.faccao||'')).replace(/\s+/g,'')));const slotTotals={'14H':[],'16H':[],'21H':[],'23H':[]};visibleRaw.forEach(r=>{const s=metricSlots(r);Object.keys(slotTotals).forEach(h=>slotTotals[h].push(s[h]))});const hourAvgs=Object.fromEntries(Object.entries(slotTotals).map(([h,v])=>[h,v.length?v.reduce((a,b)=>a+b,0)/v.length:0]));const values=visibleRaw.flatMap(r=>Object.values(metricSlots(r))),overall=values.length?values.reduce((a,b)=>a+b,0)/values.length:0,activePct=values.length?values.filter(v=>v>0).length/values.length*100:0;const top=rows[0],peak=rows.reduce((best,x)=>!best||x.a.peak.value>best.a.peak.value?x:best,null),strong=Object.entries(hourAvgs).sort((a,b)=>b[1]-a[1])[0]||['—',0],weak=Object.entries(hourAvgs).sort((a,b)=>a[1]-b[1])[0]||['—',0];
 if($('#metricActivePeriod'))$('#metricActivePeriod').innerHTML=`RECORTE ATIVO <b>${esc(metricActiveLabel())}</b>${seg?` • ${esc(seg)}`:''} • ${rows.length} organização(ões)`;
 if(!rows.length){if($('#metricOverview'))$('#metricOverview').innerHTML=`<div class="placeholder"><b>▥</b><h3>SEM MÉTRICAS NO RECORTE</h3><p>Ajuste o período, segmento ou busca.</p></div>`;if($('#metricStats'))$('#metricStats').innerHTML='';if($('#metricVisuals'))$('#metricVisuals').innerHTML='';if($('#metricRadar'))$('#metricRadar').innerHTML='';if($('#metricDistribution'))$('#metricDistribution').innerHTML='';syncMetricSelectors();renderMetricFactionDetail();renderMetricPresence();return}
 $('#metricOverview').innerHTML=`<article class="metric-hero-kpi"><span>CONTINGENTE MÉDIO</span><b>${overall.toFixed(1)}</b><small>média de todas as coletas exibidas</small></article><article class="metric-hero-kpi"><span>PRESENÇA ATIVA</span><b>${activePct.toFixed(0)}%</b><small>coletas com contingente acima de zero</small></article><article class="metric-hero-kpi"><span>MAIOR PICO</span><b>${peak?peak.a.peak.value:'—'}</b><small>${peak?`${esc(peak.f.faccao||peak.f.group)} • ${esc(peak.a.peak.hour)} • ${esc(peak.a.peak.date)}`:'—'}</small></article><article class="metric-hero-kpi"><span>HORÁRIO MAIS FORTE</span><b>${esc(strong[0])}</b><small>média ${Number(strong[1]).toFixed(1)} • mais fraco ${esc(weak[0])}</small></article>`;
 $('#metricStats').innerHTML=`<span><b>${esc(metricActiveLabel())}</b> PERÍODO</span><span><b>${rows.length}</b> ORGANIZAÇÕES</span><span><b>${visibleRaw.length}</b> DIAS / REGISTROS</span><span><b>${values.length}</b> COLETAS HORÁRIAS</span><span><b>${values.filter(v=>v===0).length}</b> REGISTROS ZERO</span>`;
 const maxHour=Math.max(1,...Object.values(hourAvgs)),top5=rows.slice(0,5),maxTop=Math.max(1,...top5.map(x=>x.a.avg));$('#metricVisuals').innerHTML=`<section class="metric-chart-card"><div class="metric-chart-head"><b>CONTINGENTE MÉDIO POR HORÁRIO</b><span>${esc(metricActiveLabel())}</span></div><div class="hour-bars">${Object.entries(hourAvgs).map(([h,v])=>`<div class="hour-col"><b>${v.toFixed(1)}</b><i style="height:${Math.max(4,v/maxHour*120)}px"></i><span>${h}</span></div>`).join('')}</div></section><section class="metric-chart-card"><div class="metric-chart-head"><b>DESTAQUES DO RECORTE</b><span>TOP 5 POR CONTINGENTE MÉDIO</span></div><div class="top-bars">${top5.map(x=>`<div class="top-bar-item"><span>${esc(x.f.faccao||x.f.group)}</span><div class="ops-track"><div class="ops-fill" style="width:${Math.max(3,x.a.avg/maxTop*100)}%"></div></div><b>${x.a.avg.toFixed(1)}</b></div>`).join('')}</div></section>`;
 renderMetricRadar(rows);renderMetricDistribution(visibleRaw);syncMetricSelectors();renderMetricFactionDetail();renderMetricPresence();if($('#metricViewRanking')?.classList.contains('active'))renderMetricAdvancedRanking();if($('#metricViewComparatives')?.classList.contains('active')){syncMetricCompareSelectors();renderMetricComparison()}
};

renderMetricFactionDetail=function(group=metricSelectedGroup()){
 document.querySelector('.rh-insights')?.remove();const sum=$('#metricFactionSummary'),table=$('#metricFactionTable');if(!sum||!table)return;if(!group){sum.innerHTML='';table.innerHTML='<div class="placeholder"><h3>SEM DADOS</h3></div>';return}const p=metricPresenceStats(group);if(!p){sum.innerHTML='';table.innerHTML='<div class="placeholder"><h3>SEM MÉTRICAS PARA ESTE GROUP</h3></div>';return}const id=metricIdentity(group,p.rows[0]),low=p.values.length?Math.min(...p.values):0,mode=metricModeValue(p.values);sum.innerHTML=`<article><span>FACÇÃO</span><b>${esc(id.faccao||'—')}</b><small>${esc(group)} • ${esc(id.segmento||'—')}</small></article><article><span>CONTINGENTE MÉDIO</span><b>${p.avg.toFixed(2)}</b><small>${esc(metricActiveLabel())}</small></article><article><span>PRESENÇA ATIVA</span><b>${p.activePct.toFixed(0)}%</b><small>${p.positiveDays}/${p.rows.length} dia(s) com presença</small></article><article><span>PICO</span><b>${p.peak.value}</b><small>${esc(p.peak.hour)} • ${esc(p.peak.date)}</small></article><article><span>MENOR REGISTRO</span><b>${low}</b><small>predominância ${esc(mode)}</small></article>`;
 const extra=document.createElement('div');extra.className='rh-insights';extra.innerHTML=`<div class="rh-grid">${Object.entries(p.hourAvg).map(([h,v])=>`<div class="rh-card"><span>CONTINGENTE MÉDIO ${h}</span><b>${v.toFixed(2)}</b></div>`).join('')}<div class="rh-card"><span>REGULARIDADE</span><b>${p.regularity.toFixed(0)}%</b><small>dias ≥ 80% da média do recorte</small></div><div class="rh-card"><span>TENDÊNCIA</span><b>${p.trend>=0?'+':''}${p.trend.toFixed(0)}%</b><small>5 dias recentes vs. 5 anteriores</small></div><div class="rh-card"><span>MELHOR DIA</span><b>${p.best?p.best.avg.toFixed(1):'—'}</b><small>${p.best?esc(metricDateLabel(p.best.r)):'—'}</small></div><div class="rh-card"><span>REGISTROS ZERO</span><b>${p.zeroCount}</b><small>entre ${p.values.length} coletas horárias</small></div></div>${p.alerts.length?p.alerts.map(a=>`<div class="rh-alert">${esc(a)}</div>`).join(''):'<div class="rh-alert">Sem alertas estatísticos relevantes no período.</div>'}`;sum.after(extra);
 table.innerHTML=`<div class="metric-date-table-title"><div><b>HISTÓRICO DIÁRIO</b><span>${esc(id.faccao||group)} • ${esc(metricActiveLabel())}</span></div></div><div class="metric-table-scroll"><table class="metric-date-table"><thead><tr><th>DATA</th><th>14H</th><th>16H</th><th>21H</th><th>23H</th><th>MÉDIA DO DIA</th></tr></thead><tbody>${p.rows.map(r=>{const s=metricSlots(r);return `<tr><td><b>${esc(metricDateLabel(r))}</b></td><td>${s['14H']}</td><td>${s['16H']}</td><td>${s['21H']}</td><td>${s['23H']}</td><td><b>${metricDayAverage(r).toFixed(2)}</b></td></tr>`}).join('')}</tbody></table></div>`;
};

renderMetricAdvancedRanking=function(){const box=$('#metricAdvancedRanking');if(!box)return;const mode=$('#metricRankingMode')?.value||'avg',seg=$('#metricSegment')?.value||'',q=alvesNorm($('#metricSearch')?.value||'');let rows=metricSummaryRows().filter(x=>(!seg||x.f.segmento===seg)&&(!q||alvesNorm([x.f.group,x.f.faccao].join(' ')).includes(q))).map(x=>({...x,x:metricPresenceStats(x.f.group)})).filter(x=>x.x);rows.sort((a,b)=>mode==='peak'?b.x.peak.value-a.x.peak.value:mode==='regularity'?b.x.regularity-a.x.regularity:b.x.avg-a.x.avg);box.innerHTML=rows.length?`<div style="margin:5px 11px 12px;opacity:.62;font-size:11px">RECORTE • ${esc(metricActiveLabel())}</div><div class="rh-ranking-row"><b>#</b><b>FACÇÃO / GROUP</b><span>MÉDIA</span><span class="rh-extra">PICO</span><span class="rh-extra">PRESENÇA</span><span class="rh-extra">TENDÊNCIA</span></div>${rows.map((r,i)=>`<div class="rh-ranking-row"><b>${i+1}</b><div><strong>${esc(r.f.faccao||r.f.group)}</strong><small style="display:block">${esc(r.f.group)} • ${esc(r.f.segmento||'—')}</small></div><span><b>${r.x.avg.toFixed(2)}</b></span><span class="rh-extra">${r.x.peak.value}</span><span class="rh-extra">${r.x.activePct.toFixed(0)}%</span><span class="rh-extra">${r.x.trend>=0?'+':''}${r.x.trend.toFixed(0)}%</span></div>`).join('')}`:'<div class="placeholder"><h3>SEM DADOS</h3></div>'};

renderMetricComparison=function(){const ga=$('#metricCompareA')?.value,gb=$('#metricCompareB')?.value,box=$('#metricCompareResult');if(!box||!ga||!gb)return;const a=metricPresenceStats(ga),b=metricPresenceStats(gb);if(!a||!b){box.innerHTML='<div class="placeholder"><h3>SEM DADOS PARA COMPARAR</h3></div>';return}const ia=metricIdentity(ga,a.rows[0]),ib=metricIdentity(gb,b.rows[0]);const rows=[['Contingente médio',a.avg.toFixed(2),b.avg.toFixed(2)],['Presença ativa',a.activePct.toFixed(0)+'%',b.activePct.toFixed(0)+'%'],['Pico',a.peak.value,b.peak.value],['Regularidade',a.regularity.toFixed(0)+'%',b.regularity.toFixed(0)+'%'],['Tendência',(a.trend>=0?'+':'')+a.trend.toFixed(0)+'%',(b.trend>=0?'+':'')+b.trend.toFixed(0)+'%'],...['14H','16H','21H','23H'].map(h=>['Contingente médio '+h,a.hourAvg[h].toFixed(2),b.hourAvg[h].toFixed(2)])];box.innerHTML=`<div class="rh-grid"><div class="rh-card"><span>FACÇÃO A</span><b>${esc(ia.faccao||ga)}</b><small>${esc(ga)}</small></div><div class="rh-card"><span>FACÇÃO B</span><b>${esc(ib.faccao||gb)}</b><small>${esc(gb)}</small></div><div class="rh-card"><span>DIFERENÇA DE CONTINGENTE</span><b>${Math.abs(a.avg-b.avg).toFixed(2)}</b><small>${a.avg>=b.avg?esc(ia.faccao||ga):esc(ib.faccao||gb)} à frente</small></div><div class="rh-card"><span>RECORTE</span><b style="font-size:15px">${esc(metricActiveLabel())}</b></div></div><table class="rh-compare-table"><thead><tr><th>INDICADOR</th><th>${esc(ia.faccao||ga)}</th><th>${esc(ib.faccao||gb)}</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r[0]}</td><td><b>${r[1]}</b></td><td><b>${r[2]}</b></td></tr>`).join('')}</tbody></table>`};

metricReportData=function(group,period=metricPeriodKey){
 let a;if(metricFilterMode==='range')a=metricAnalysis(group);else{const prev=metricPeriodKey;metricPeriodKey=period||prev;a=metricAnalysis(group);metricPeriodKey=prev}if(!a)return null;const id=metricIdentity(group,a.rows[0]),vals=a.rows.flatMap(r=>Object.values(metricSlots(r)));return {group,id,a,low:vals.length?Math.min(...vals):0,mode:metricModeValue(vals),range:metricPeriodRange(a.rows),period:metricFilterMode==='range'?'RANGE':period,periodLabel:metricFilterMode==='range'?metricActiveLabel():metricPeriodLabel(period)};
};
buildMetricReportHtml=function(data,printMode=false){if(!data)return '<div class="placeholder"><h3>SEM DADOS PARA O RELATÓRIO</h3></div>';const {group,id,a,low,mode,range,periodLabel}=data,p=metricPresenceStats(group);return `<article class="monthly-report${printMode?' print-report':''}"><header><div><span>HIGH ROLEPLAY • CENTRAL RH DE MÉTRICAS</span><h2>RELATÓRIO DE CONTINGENTE E PRESENÇA</h2><p>${esc(periodLabel)} • período ${esc(range)}</p></div><div class="report-badge">RH</div></header><section class="report-ident"><div><span>FACÇÃO</span><b>${esc(id.faccao||'—')}</b></div><div><span>GROUP</span><b>${esc(group)}</b></div><div><span>SEGMENTO</span><b>${esc(id.segmento||'—')}</b></div><div><span>QG</span><b>${esc(id.qg||'—')}</b></div><div><span>LÍDER</span><b>${esc(id.lider||'—')}</b></div></section><section class="report-kpis"><div><span>CONTINGENTE MÉDIO</span><b>${a.avg.toFixed(2)}</b></div><div><span>PRESENÇA ATIVA</span><b>${p?p.activePct.toFixed(0)+'%':'—'}</b></div><div><span>PICO</span><b>${a.peak.value}</b><small>${esc(a.peak.hour)} • ${esc(a.peak.date)}</small></div><div><span>MENOR</span><b>${low}</b></div><div><span>DIAS ANALISADOS</span><b>${a.rows.length}</b></div></section><section><h3>REGISTROS DIÁRIOS</h3><table><thead><tr><th>Data</th><th>14H</th><th>16H</th><th>21H</th><th>23H</th><th>Média</th></tr></thead><tbody>${a.rows.map(r=>{const s=metricSlots(r);return `<tr><td>${esc(metricDateLabel(r))}</td><td>${s['14H']}</td><td>${s['16H']}</td><td>${s['21H']}</td><td>${s['23H']}</td><td>${metricDayAverage(r).toFixed(2)}</td></tr>`}).join('')}</tbody></table></section><footer>Gerado pelo High OS • ${new Date().toLocaleString('pt-BR')}</footer></article>`};

function syncMetricRangeUi(){const month=$('#metricMonthWrap'),range=$('#metricRangeWrap'),bm=$('#metricModeMonth'),br=$('#metricModeRange');month?.classList.toggle('hidden',metricFilterMode==='range');range?.classList.toggle('hidden',metricFilterMode!=='range');bm?.classList.toggle('active',metricFilterMode==='month');br?.classList.toggle('active',metricFilterMode==='range');const reportPeriod=$('#metricReportPeriod')?.closest('label');if(reportPeriod)reportPeriod.style.display=metricFilterMode==='range'?'none':'';if($('#metricActivePeriod'))$('#metricActivePeriod').innerHTML=`RECORTE ATIVO <b>${esc(metricActiveLabel())}</b>`}
function initMetricRangeDefaults(){const b=metricAllDateBounds();if(!metricRangeFrom&&b.min)metricRangeFrom=metricIsoDate(b.min);if(!metricRangeTo&&b.max)metricRangeTo=metricIsoDate(b.max);if($('#metricDateFrom'))$('#metricDateFrom').value=metricRangeFrom;if($('#metricDateTo'))$('#metricDateTo').value=metricRangeTo;syncMetricRangeUi()}
const _refreshMetricPeriodOptionsV715=refreshMetricPeriodOptions;refreshMetricPeriodOptions=function(){_refreshMetricPeriodOptionsV715();initMetricRangeDefaults()};
const _switchMetricCenterViewV715=switchMetricCenterView;switchMetricCenterView=function(view='overview'){_switchMetricCenterViewV715(view);if(view==='presence')renderMetricPresence()};
$('#metricModeMonth')?.addEventListener('click',()=>{metricFilterMode='month';syncMetricRangeUi();renderMetrics()});
$('#metricModeRange')?.addEventListener('click',()=>{metricFilterMode='range';initMetricRangeDefaults();renderMetrics()});
$('#metricApplyRange')?.addEventListener('click',()=>{const a=$('#metricDateFrom')?.value,b=$('#metricDateTo')?.value;if(!a||!b)return alert('Informe as duas datas do intervalo.');if(a>b)return alert('A data inicial não pode ser maior que a data final.');metricRangeFrom=a;metricRangeTo=b;metricFilterMode='range';syncMetricRangeUi();renderMetrics()});
$('#metricPresenceSort')?.addEventListener('change',renderMetricPresence);
$('#metricPresenceGrid')?.addEventListener('click',e=>{const card=e.target.closest('[data-open-group]');if(!card)return;switchMetricCenterView('faction');const sel=$('#metricFactionSelect');if(sel){sel.value=card.dataset.openGroup;renderMetricFactionDetail(card.dataset.openGroup)}});
initMetricRangeDefaults();

// ===== HIGH OS V7.15 · GESTÃO ADMINISTRATIVA COMPLETA =====
let groupAdminView='cards';
function hasTextV715(v){return String(v??'').trim().length>0}
function firstV715(...v){return v.find(hasTextV715)||''}
function groupAdminSnapshot(f={}){
 const b=f.beneficios||{},t=mergedTechProfile(f)||{},op=t.operacional||{},ex=t.estruturaExtra||{},gar=Array.isArray(op.garagens)?op.garagens:[],heli=Array.isArray(op.helipontos)?op.helipontos:[];
 const mainCds=firstV715(f.cds,op.localizacao?.cdsPrincipal,ex.coordenadaBase);
 const craft=firstV715(t.craft?.cds,b.craft); const farm=firstV715(t.farm?.cds,t.rota?.inicio,b.farm);
 const loja=firstV715(op.lojaFac?.cds,b.shopExclusivo,ex.shopDeluxe); const bar=firstV715(op.bar?.cds,ex.bar); const barb=firstV715(op.barbearia?.cds,b.barbearia); const tattoo=firstV715(op.tatuagem?.cds,b.tatuagem); const roupas=firstV715(op.roupas?.cds,b.lojaRoupas); const bau=firstV715(b.bau); const radio=firstV715(b.radio);
 const garage=gar.some(g=>hasTextV715(g.blip)||hasTextV715(g.spawn))||hasTextV715(b.garagemPublicaBlip)||hasTextV715(b.garagemVipBlip)||hasTextV715(ex.garagemDeluxe);
 const helipad=heli.some(h=>hasTextV715(h.blip)||hasTextV715(h.spawn))||hasTextV715(b.helipontoBlip);
 const telao=!!(op.telao?.ativo||b.telao||hasTextV715(op.telao?.cds)||hasTextV715(b.telaoCds));
 const checks=[['CDS PRINCIPAL',!!mainCds],['QG / LOCAL',hasTextV715(f.qg)],['CRAFT',!!craft],['FARM / ROTA',!!farm],['GARAGEM',!!garage],['LOJA',!!loja],['BARBEARIA',!!barb],['ROUPAS',!!roupas],['BAÚ',!!bau],['RÁDIO',!!radio]];
 const done=checks.filter(x=>x[1]).length,score=Math.round(done/checks.length*100),missing=checks.filter(x=>!x[1]).map(x=>x[0]);
 const resources=[['CRAFT',!!craft],['FARM',!!farm],['GARAGEM',!!garage],['HELIPONTO',!!helipad],['LOJA',!!loja],['BAR',!!bar],['BARBEARIA',!!barb],['TATUAGEM',!!tattoo],['ROUPAS',!!roupas],['TELÃO',!!telao]];
 return {mainCds,craft,farm,loja,bar,barb,tattoo,roupas,bau,radio,garage,helipad,telao,score,missing,resources,structureCount:resources.filter(x=>x[1]).length};
}
function groupAdminFilter(f){
 const q=($('#facSearch')?.value||'').toLowerCase(),seg=$('#facSegment')?.value||'',st=$('#facStatus')?.value||'',comp=$('#facCompleteness')?.value||'',a=groupAdminSnapshot(f);
 if(seg&&f.segmento!==seg)return false;if(st&&f.status!==st)return false;if(q&&![f.group,f.faccao,f.qg,f.lider,f.staff,f.produto,a.mainCds].join(' ').toLowerCase().includes(q))return false;
 if(comp==='PENDENTE'&&a.score>=80)return false;if(comp==='COMPLETO'&&a.score<80)return false;if(comp==='SEM_CDS'&&a.mainCds)return false;return true;
}
renderFaccoes=function(){
 const listBox=$('#facList'),audit=$('#facAudit');if(!listBox)return;const all=faccoes.filter(f=>!f.removido),filtered=all.filter(groupAdminFilter),snap=all.map(f=>({f,a:groupAdminSnapshot(f)}));
 const occupied=all.filter(f=>f.status==='ATIVA').length,vacant=all.length-occupied,pending=snap.filter(x=>x.a.score<80).length,noCds=snap.filter(x=>!x.a.mainCds).length,avg=snap.length?snap.reduce((n,x)=>n+x.a.score,0)/snap.length:0;
 if($('#facOverview'))$('#facOverview').innerHTML=`<div class="ops-kpis"><article class="ops-kpi purple"><span>GROUPS OPERACIONAIS</span><b>${all.length}</b><small>patrimônio cadastrado</small></article><article class="ops-kpi good"><span>OCUPADOS</span><b>${occupied}</b><small>com facção atualmente</small></article><article class="ops-kpi"><span>VAGOS</span><b>${vacant}</b><small>disponíveis para entrega</small></article><article class="ops-kpi attention"><span>PENDÊNCIAS</span><b>${pending}</b><small>cadastro abaixo de 80%</small></article><article class="ops-kpi info"><span>QUALIDADE DA BASE</span><b>${avg.toFixed(0)}%</b><small>${noCds} sem CDS principal</small></article></div>`;
 if($('#facStats'))$('#facStats').innerHTML=`<span><b>${filtered.length}</b> EXIBIDOS</span><span>VISÃO <b>${groupAdminView==='audit'?'AUDITORIA':'PAINEL'}</b></span><span><b>${snap.reduce((n,x)=>n+x.a.structureCount,0)}</b> ESTRUTURAS MAPEADAS</span>`;
 if(groupAdminView==='audit'){
   listBox.classList.add('hidden');audit?.classList.remove('hidden');
   if(audit)audit.innerHTML=`<div class="audit-head"><span>GROUP</span><span>OCUPAÇÃO / LOCAL</span><span>CADASTRO</span><span>PENDÊNCIAS</span><span>AÇÃO</span></div>${filtered.slice().sort((x,y)=>groupAdminSnapshot(x).score-groupAdminSnapshot(y).score).map(f=>{const a=groupAdminSnapshot(f);return `<div class="audit-row"><div><b>${esc(f.group)}</b><small style="display:block;opacity:.55">${esc(f.segmento||'—')}</small></div><div><b>${esc(f.faccao||'VAGO')}</b><small style="display:block;opacity:.55">${esc(f.qg||'SEM LOCAL')}</small></div><div><span class="audit-score">${a.score}%</span><div class="admin-progress"><i style="width:${a.score}%"></i></div></div><div class="audit-missing">${a.missing.length?esc(a.missing.join(' • ')):'CADASTRO BEM ESTRUTURADO'}</div><button class="mini-btn admin-open-group" data-id="${esc(f.id)}">ABRIR</button></div>`}).join('')||'<div class="admin-empty">Nenhum Group encontrado neste filtro.</div>';
 }else{
   audit?.classList.add('hidden');listBox.classList.remove('hidden');
   listBox.innerHTML=filtered.length?filtered.map(f=>{const a=groupAdminSnapshot(f);const chips=a.resources.filter(x=>x[1]).slice(0,6);return `<article class="fac-card admin-group-card" data-id="${esc(f.id)}"><div class="fac-card-head"><div><div class="group-kicker">${esc(f.segmento||'GROUP')}</div><h3>${esc(f.group)}</h3></div><span class="status-chip ${f.status==='ATIVA'?'ativa':'inativa'}">${f.status==='ATIVA'?'OCUPADO':'VAGO'}</span></div><div class="fac-name">${esc(f.faccao||'— DISPONÍVEL —')}</div><div class="muted">${esc(f.qg||'LOCAL NÃO CADASTRADO')}</div><div class="group-health"><div class="group-health-track"><div class="group-health-fill" style="width:${a.score}%"></div></div><b>${a.score}%</b></div><div class="resource-chips">${chips.map(([n])=>`<span class="resource-chip on">${esc(n)}</span>`).join('')}${a.missing.slice(0,2).map(n=>`<span class="resource-chip missing">FALTA ${esc(n)}</span>`).join('')}</div><div class="org-readiness"><div><span>ESTRUTURAS</span><b>${a.structureCount}</b></div><div><span>CDS PRINCIPAL</span><b>${a.mainCds?'OK':'PENDENTE'}</b></div><div><span>LIDERANÇA</span><b>${f.lider?'OK':'—'}</b></div></div><div class="admin-card-actions"><button class="mini-btn admin-open-group" data-id="${esc(f.id)}">PERFIL TÉCNICO</button><button class="mini-btn admin-copy-cds" data-cds="${esc(a.mainCds)}">COPIAR CDS</button><button class="mini-btn req-from-fac" data-group="${esc(f.group)}">SOLICITAÇÃO</button></div></article>`}).join(''):'<div class="placeholder"><b>◆</b><h3>NENHUM GROUP ENCONTRADO</h3><p>Ajuste os filtros administrativos.</p></div>';
 }
 document.querySelectorAll('.admin-open-group').forEach(b=>b.onclick=e=>{e.stopPropagation();openFac(b.dataset.id)});document.querySelectorAll('.admin-group-card').forEach(c=>c.onclick=e=>{if(e.target.closest('button'))return;openFac(c.dataset.id)});document.querySelectorAll('.req-from-fac').forEach(b=>b.onclick=e=>{e.stopPropagation();openRequestModal('',b.dataset.group)});document.querySelectorAll('.admin-copy-cds').forEach(b=>b.onclick=e=>{e.stopPropagation();if(!b.dataset.cds)return alert('Este Group ainda não possui CDS principal cadastrada.');copyText(b.dataset.cds,b)});
};
function orgAdminSnapshot(o={}){const current=faccoes.find(f=>alvesNorm(f.faccao)===alvesNorm(o.nome)),checks=[['LÍDER',hasTextV715(o.lider||current?.lider)],['CONTATO',hasTextV715(o.contato)],['DISCORD',hasTextV715(o.discord)],['DESDE',hasTextV715(o.desde||current?.dataEntrega)],['STATUS',hasTextV715(o.status)],['GROUP',!!(o.groupAtual||current?.group)]];const score=Math.round(checks.filter(x=>x[1]).length/checks.length*100);return {current,score,missing:checks.filter(x=>!x[1]).map(x=>x[0])}}
renderOrganizations=function(){
 const box=$('#orgList');if(!box)return;const all=derivedOrganizations(),q=($('#orgSearch')?.value||'').toLowerCase(),st=$('#orgStatus')?.value||'',comp=$('#orgCompleteness')?.value||'';let list=all.filter(o=>(!st||o.status===st)&&(!q||[o.nome,o.lider,o.groupAtual,o.segmentoAtual,o.contato,o.discord].join(' ').toLowerCase().includes(q)));if(comp)list=list.filter(o=>comp==='COMPLETO'?orgAdminSnapshot(o).score>=80:orgAdminSnapshot(o).score<80);
 const active=all.filter(o=>o.groupAtual&&o.status!=='INATIVA').length,sem=all.filter(o=>!o.groupAtual&&o.status!=='INATIVA').length,inativas=all.filter(o=>o.status==='INATIVA').length,pending=all.filter(o=>orgAdminSnapshot(o).score<80).length;
 if($('#orgOverview'))$('#orgOverview').innerHTML=`<div class="ops-kpis"><article class="ops-kpi purple"><span>FACÇÕES CADASTRADAS</span><b>${all.length}</b><small>registro histórico preservado</small></article><article class="ops-kpi good"><span>OCUPANDO GROUP</span><b>${active}</b><small>organizações ativas</small></article><article class="ops-kpi warn"><span>SEM GROUP</span><b>${sem}</b><small>aguardando ocupação</small></article><article class="ops-kpi attention"><span>CADASTROS PENDENTES</span><b>${pending}</b><small>liderança/contato/dados</small></article><article class="ops-kpi"><span>INATIVAS</span><b>${inativas}</b><small>mantidas no histórico</small></article></div>`;
 if($('#orgStats'))$('#orgStats').innerHTML=`<span><b>${list.length}</b> EXIBIDAS</span><span><b>${active}</b> OCUPAÇÕES ATUAIS</span><span>CADASTRO E OCUPAÇÃO SÃO <b>INDEPENDENTES</b></span>`;
 box.innerHTML=list.length?list.map(o=>{const a=orgAdminSnapshot(o),c=a.current,group=o.groupAtual||c?.group||'',segment=o.segmentoAtual||c?.segmento||'',qg=o.qgAtual||c?.qg||'';return `<article class="org-card"><div class="org-card-head"><div><div class="group-kicker">${esc(segment||'ORGANIZAÇÃO')}</div><h3>${esc(o.nome||'SEM NOME')}</h3></div><span class="status-chip ${group?'ativa':'inativa'}">${group?'COM GROUP':'SEM GROUP'}</span></div><div class="org-group-link"><span>OCUPAÇÃO ATUAL</span><b>${esc(group||'—')}</b><small>${esc(qg||'Nenhum QG vinculado')}</small></div><div class="group-health"><div class="group-health-track"><div class="group-health-fill" style="width:${a.score}%"></div></div><b>${a.score}%</b></div><div class="org-readiness"><div><span>LÍDER</span><b>${esc(o.lider||c?.lider||'PENDENTE')}</b></div><div><span>CONTATO</span><b>${esc(o.contato||'—')}</b></div><div><span>DESDE</span><b>${esc(o.desde||c?.dataEntrega||'—')}</b></div></div>${a.missing.length?`<div class="resource-chips">${a.missing.slice(0,3).map(n=>`<span class="resource-chip missing">FALTA ${esc(n)}</span>`).join('')}</div>`:''}<div class="admin-card-actions"><button class="mini-btn open-org" data-name="${esc(o.nome)}">CADASTRO</button>${group?`<button class="mini-btn org-open-group" data-group="${esc(group)}">GROUP</button><button class="mini-btn org-open-metric" data-group="${esc(group)}">MÉTRICAS</button>`:''}</div></article>`}).join(''):'<div class="placeholder"><b>♜</b><h3>NENHUMA FACÇÃO ENCONTRADA</h3><p>Ajuste a busca ou os filtros.</p></div>';
 document.querySelectorAll('.open-org').forEach(b=>b.onclick=e=>{e.stopPropagation();openOrganizationByName(b.dataset.name)});document.querySelectorAll('.org-open-group').forEach(b=>b.onclick=e=>{e.stopPropagation();const f=faccoes.find(x=>x.group===b.dataset.group);if(f)openFac(f.id)});document.querySelectorAll('.org-open-metric').forEach(b=>b.onclick=e=>{e.stopPropagation();activateAppPage('metricas');switchMetricCenterView('faction');setTimeout(()=>{const s=$('#metricFactionSelect');if(s){s.value=b.dataset.group;renderMetricFactionDetail(b.dataset.group)}},80)});
};
function initAdminManagementV715(){
 $('#groupViewCards')?.addEventListener('click',()=>{groupAdminView='cards';$('#groupViewCards').classList.add('active');$('#groupViewAudit')?.classList.remove('active');renderFaccoes()});
 $('#groupViewAudit')?.addEventListener('click',()=>{groupAdminView='audit';$('#groupViewAudit').classList.add('active');$('#groupViewCards')?.classList.remove('active');renderFaccoes()});
 $('#facCompleteness')?.addEventListener('change',renderFaccoes);$('#orgCompleteness')?.addEventListener('change',renderOrganizations);
}
