...
function dashboardHealth(alerts=[]){const open=alerts.filter(x=>!['CONCLUIDO','LIMPO'].includes(x.state||'PENDENTE'));if(open.some(x=>x.level==='CRÍTICO'))return 'CRÍTICO';if(open.length)return 'ATENÇÃO';return 'NORMAL'}
...
function anomalyIsCleared(group,date){const st=dashboardAlertStates.find(x=>x.id===anomalyAlertId(group));if(!st||st.status!=='LIMPO')return false;const cleared=st.metricAt?.toDate?st.metricAt.toDate():st.metricAt?new Date(st.metricAt):null;return !!(cleared&&date&&date<=cleared)}
async function clearVacantMetricAlert(group,date){if(!canEditModule('dashboard'))return permissionDeniedMessage('dashboard',true);const id=anomalyAlertId(group),metricAt=date instanceof Date?date:new Date(date);try{await setDoc(doc(db,'highos','data','alertas_dashboard',id),{tipo:'VAGO_COM_METRICA',group,status:'LIMPO',metricAt,updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{sessionId:currentSessionId||'',tipo:'ALERTA_DASHBOARD',group,descricao:'Alerta de Group vago com métrica removido do Dashboard',depois:{group,status:'LIMPO',metricAt:metricAt.toISOString()},usuario:currentUser.email,data:serverTimestamp()});await loadDashboardAlertStates();renderCommandDashboard()}catch(e){alert('Erro ao limpar o alerta: '+e.message)}}
...
async function logout(reason='LOGOUT'){await closeCurrentSession(reason);try{stopChat()}catch(e){}await signOut(auth)}
...
const SYSTEM_MODULES=[
 ...
 {id:'faccoes',label:'Organizações',desc:'Group permanente, ocupação, divulgação, entrega, estrutura, rota e histórico'},
...
const INTERNAL_MODULE_PARENT={'group-profile':'faccoes','group-settings':'faccoes','organizacoes':'faccoes','org-profile':'faccoes','disponiveis':'faccoes','entregas':'faccoes'};
...
function moduleForElement(el){const page=el?.closest?.('.page');if(page)return pageModule(page.id.replace('page-',''));const modal=el?.closest?.('.modal')?.id||'';const map={facModal:'faccoes',recipeEditorModal:'faccoes',farmEditorModal:'faccoes',movementModal:'faccoes',newDeliveryModal:'faccoes',reqModal:'solicitacoes',craftRequestModal:'faccoes',metricSourceModal:'metricas',metricImportModal:'metricas',orgModal:'faccoes',userModal:'administracao',auditSessionModal:'administracao',facSheetDiffModal:'administracao'};return map[modal]||pageModule(document.querySelector('.page.active')?.id?.replace('page-','')||'dashboard')}
...
function vacantMetricAnomalies(){
 ...
 return [...by.values()].filter(x=>!anomalyIsCleared(x.group,x.date)).map(x=>{const f=faccoes.find(z=>alvesNorm(z.group)===alvesNorm(x.group));const lastDelivery=historico.filter(h=>h.tipo==='ENTREGA_GROUP'&&alvesNorm(h.group)===alvesNorm(x.group)).sort((a,b)=>historyMillis(b)-historyMillis(a))[0];return {...x,qg:f?.qg||'',staff:f?.staff||'',hasExtract:!!lastDelivery};}).sort((a,b)=>b.date-a.date);
}
...
function renderCommandDashboard(){
 ...
 const active=faccoes.filter(f=>f.status==='ATIVA'&&f.faccao),vacant=faccoes.filter(f=>f.status!=='ATIVA'||!f.faccao),weekly=weeklyContingentAlerts(),visibleWeekly=weekly.filter(x=>x.state!=='LIMPO'),openAlerts=visibleWeekly.filter(x=>x.state!=='CONCLUIDO'),critical=openAlerts.filter(x=>x.level==='CRÍTICO').length;
 ...
 box.querySelectorAll('[data-go]').forEach(b=>b.onclick=()=>dashboardGo(b.dataset.go));box.querySelector('[data-open-management]')?.addEventListener('click',()=>{activateAppPage('metricas');switchMetricCenterView('management');renderIllegalManagement()});box.querySelector('[data-open-general-report]')?.addEventListener('click',mgmtOpenRichReport);box.querySelectorAll('[data-alert-group]').forEach(b=>b.onclick=()=>openMetricForGroup(b.dataset.alertGroup));box.querySelectorAll('[data-alert-state]').forEach(b=>b.onclick=e=>{e.stopPropagation();setDashboardAlertState(b.dataset.group,b.dataset.week,b.dataset.alertState)});box.querySelectorAll('[data-anomaly-group]').forEach(b=>b.onclick=()=>{activateAppPage('faccoes');const f=faccoes.find(x=>alvesNorm(x.group)===alvesNorm(b.dataset.anomalyGroup));if(f)openFac(f.id)});box.querySelectorAll('[data-clear-anomaly]').forEach(b=>b.onclick=e=>{e.stopPropagation();clearVacantMetricAlert(b.dataset.clearAnomaly,new Date(b.dataset.anomalyDate))});
}
...
let spotifyMiniState=null,spotifyProgressTimer=null,spotifyPlaylistsLoaded=false;
function spotifyTime(ms=0){const sec=Math.max(0,Math.floor(Number(ms||0)/1000));return `${Math.floor(sec/60)}:${String(sec%60).padStart(2,'0')}`}
function spotifyRenderState(state){if(!state)return;spotifyMiniState=state;const t=state.track_window?.current_track||null,artists=t?.artists?.map(a=>a.name).join(', ')||'High OS',cover=t?.album?.images?.[0]?.url||'';const title=$('#spotifyNowTitle'),artist=$('#spotifyNowArtist'),miniTitle=$('#spotifyMiniTitle'),miniArtist=$('#spotifyMiniArtist'),miniAlbum=$('#spotifyMiniAlbum'),g=$('#spotifyPlayGlyph'),play=$('#spotifyPlayBtn'),coverBox=$('#spotifyCover');if(title)title.textContent=t?.name||'Spotify';if(artist)artist.textContent=artists;if(miniTitle)miniTitle.textContent=t?.name||'Nenhuma música tocando';if(miniArtist)miniArtist.textContent=artists;if(miniAlbum)miniAlbum.textContent=t?.album?.name||'';if(g)g.textContent=state.paused?'▶':'❚❚';if(play)play.textContent=state.paused?'▶':'❚❚';if(coverBox)coverBox.innerHTML=cover?`<img src="${esc(cover)}" alt="Capa de ${esc(t?.album?.name||t?.name||'música')}">`:'<span>♫</span>';spotifyUpdateProgress(state.position||0,state.duration||0)}
function spotifyUpdateProgress(position=0,duration=0){const p=$('#spotifyProgress'),elapsed=$('#spotifyElapsed'),dur=$('#spotifyDuration');if(elapsed)elapsed.textContent=spotifyTime(position);if(dur)dur.textContent=spotifyTime(duration);if(p&&!p.matches(':active'))p.value=duration>0?String(Math.max(0,Math.min(1000,Math.round(position/duration*1000)))):'0'}
function spotifyStartProgress(){clearInterval(spotifyProgressTimer);spotifyProgressTimer=setInterval(()=>{if(!spotifyMiniState||spotifyMiniState.paused)return;spotifyMiniState={...spotifyMiniState,position:Math.min(spotifyMiniState.duration||0,(spotifyMiniState.position||0)+1000)};spotifyUpdateProgress(spotifyMiniState.position,spotifyMiniState.duration)},1000)}
function spotifyCreatePlayer(){if(!window.Spotify||!spotifyAccessToken||spotifyPlayer)return;const savedVolume=Math.max(0,Math.min(100,Number(localStorage.getItem('highos_spotify_volume')??45)));spotifyPlayer=new Spotify.Player({name:'High OS Mini Spotify',getOAuthToken:cb=>cb(spotifyAccessToken),volume:savedVolume/100});const vol=$('#spotifyVolume'),volText=$('#spotifyVolumeValue');if(vol){vol.value=String(savedVolume);if(volText)volText.textContent=`${Math.round(savedVolume)}%`}spotifyPlayer.addListener('ready',async({device_id})=>{spotifyDeviceId=device_id;renderSpotifyAuthUI();try{await spotifyApi('/me/player',{method:'PUT',body:JSON.stringify({device_ids:[device_id],play:false})});const state=await spotifyPlayer.getCurrentState();if(state)spotifyRenderState(state)}catch(e){console.warn('Transfer playback',e)}});spotifyPlayer.addListener('player_state_changed',state=>{if(!state)return;spotifyRenderState(state);spotifyStartProgress()});spotifyPlayer.addListener('authentication_error',({message})=>console.warn(message));spotifyPlayer.addListener('account_error',({message})=>{const s=$('#spotifyRealStatus');if(s)s.textContent='Spotify informou restrição de conta/Playback: '+message;const a=$('#spotifyDockAuth');if(a)a.innerHTML=`<span class="spotify-error">Playback indisponível: ${esc(message)}</span>`});spotifyPlayer.connect();spotifyStartProgress()}
...
async function spotifySearch(){const q=$('#spotifySearchInput')?.value.trim();if(!q)return;const box=$('#spotifySearchResults');if(box)box.innerHTML='<small>Buscando...</small>';try{const x=await spotifyApi('/search?type=track&limit=10&q='+encodeURIComponent(q));const items=x?.tracks?.items||[];if(box)box.innerHTML=items.map(t=>`<div class="spotify-track-row"><img src="${esc(t.album?.images?.at(-1)?.url||'')}" alt=""><span><b>${esc(t.name)}</b><small>${esc(t.artists?.map(a=>a.name).join(', ')||'')} • ${esc(t.album?.name||'')}</small></span><div class="spotify-track-actions"><button type="button" data-spotify-play="${esc(t.uri)}" title="Tocar agora">▶</button><button type="button" data-spotify-queue="${esc(t.uri)}" title="Adicionar à fila">＋</button></div></div>`).join('')||'<small>Nenhuma faixa encontrada.</small>';box?.querySelectorAll('[data-spotify-play]').forEach(b=>b.onclick=()=>spotifyPlayUri(b.dataset.spotifyPlay));box?.querySelectorAll('[data-spotify-queue]').forEach(b=>b.onclick=async()=>{await spotifyQueueUri(b.dataset.spotifyQueue);b.textContent='✓';setTimeout(()=>b.textContent='＋',1200)})}catch(e){if(box)box.innerHTML=`<small>${esc(e.message)}</small>`}}
...
async function spotifyQueueUri(uri){if(!spotifyDeviceId)return alert('Player ainda não está pronto.');try{await spotifyApi('/me/player/queue?uri='+encodeURIComponent(uri)+'&device_id='+encodeURIComponent(spotifyDeviceId),{method:'POST'})}catch(e){alert('Não foi possível adicionar à fila: '+e.message)}}
async function spotifyLoadPlaylists(force=false){const box=$('#spotifyPlaylistResults');if(!box||(!force&&spotifyPlaylistsLoaded))return;if(box)box.innerHTML='<small>Carregando playlists...</small>';try{const x=await spotifyApi('/me/playlists?limit=20');const items=x?.items||[];spotifyPlaylistsLoaded=true;box.innerHTML=items.map(p=>`<button type="button" class="spotify-playlist-row" data-spotify-context="${esc(p.uri)}"><span class="spotify-playlist-cover">${p.images?.[0]?.url?`<img src="${esc(p.images[0].url)}" alt="">`:'♫'}</span><span><b>${esc(p.name)}</b><small>${Number(p.tracks?.total||0)} músicas</small></span><span>▶</span></button>`).join('')||'<small>Nenhuma playlist encontrada.</small>';box.querySelectorAll('[data-spotify-context]').forEach(b=>b.onclick=()=>spotifyPlayContext(b.dataset.spotifyContext))}catch(e){box.innerHTML=`<small>${esc(e.message)}</small>`}}
async function spotifyPlayContext(uri){if(!spotifyDeviceId)return alert('Player ainda não está pronto.');try{await spotifyApi('/me/player/play?device_id='+encodeURIComponent(spotifyDeviceId),{method:'PUT',body:JSON.stringify({context_uri:uri})})}catch(e){alert(e.message)}}
function spotifySetTab(name='search'){document.querySelectorAll('[data-spotify-tab]').forEach(b=>b.classList.toggle('active',b.dataset.spotifyTab===name));$('#spotifyTabSearch')?.classList.toggle('active',name==='search');$('#spotifyTabPlaylists')?.classList.toggle('active',name==='playlists');if(name==='playlists')spotifyLoadPlaylists()}
...
function chatMeetingHtml(m){if(!m.callId)return '';const mine=String(m.email||'').toLowerCase()===String(currentUser?.email||'').toLowerCase();return `<div class="chat-call-card"><div class="chat-call-icon">☎</div><div class="chat-call-info"><b>${mine?'Você iniciou uma chamada':`${esc(m.nome||'Usuário')} iniciou uma chamada`}</b><small>${m.callMode==='video'?'Vídeo':'Áudio'} • High Call WebRTC</small></div></div>`}
...
function subscribeChatConversation(){
 ...
 if(!chatRecipientEmail){chatItems=[];renderChatMessages([]);return}
 try{
  chatUnsubscribe=onSnapshot(chatConversationQuery(),qs=>{
   const items=qs.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>{
   ...
   if(box)box.innerHTML=`<div class="chat-empty">Nao foi possivel carregar a conversa: ${esc(e.message)}</div>`;
  });
...
const HIGH_RTC_CONFIG={iceServers:[{urls:['stun:stun.l.google.com:19302','stun:stun1.l.google.com:19302']}]};
function callDocRef(id){return doc(db,'highos','data','call_signals',id)}
function rtcDesc(v){return v?{type:v.type,sdp:v.sdp}:null}
function rtcCandidate(v){return v?.toJSON?v.toJSON():v}
function callUiStatus(text){const e=$('#teamCallConnectionStatus');if(e)e.textContent=text||''}
function setCallButtons(){const mic=$('#teamCallMic'),cam=$('#teamCallCam');const at=activeLocalStream?.getAudioTracks?.()[0],vt=activeLocalStream?.getVideoTracks?.()[0];if(mic)mic.textContent=at?.enabled===false?'🎙 ATIVAR MIC':'🎙 MIC';if(cam){cam.disabled=!vt;cam.textContent=vt?.enabled===false?'◉ ATIVAR CÂMERA':'◉ CÂMERA'}}
function showCallOverlay(title='HIGH CALL',mode='audio',incoming=false){const o=$('#teamMeetingOverlay');if(!o)return;activeCallMode=mode;$('#teamMeetingTitle').textContent=title;$('#teamMeetingRoomLabel').textContent=mode==='video'?'Chamada direta • áudio + vídeo • WebRTC':'Chamada direta • áudio • WebRTC';o.classList.remove('hidden');o.classList.toggle('incoming',incoming);setCallButtons()}
async function prepareLocalMedia(mode='audio'){
 ...
 const stream=await navigator.mediaDevices.getUserMedia({audio:true,video:mode==='video'});activeLocalStream=stream;
 const local=$('#teamCallLocalVideo');if(local){local.srcObject=stream;local.classList.toggle('hidden',mode!=='video')}
 setCallButtons();return stream;
}
function bindRemoteStream(){activeRemoteStream=new MediaStream();const remote=$('#teamCallRemoteVideo');if(remote){remote.srcObject=activeRemoteStream;remote.classList.remove('hidden')}}
function createPeer(callId,role){
 seenRemoteCandidates=new Set();const pc=new RTCPeerConnection(HIGH_RTC_CONFIG);activePeer=pc;bindRemoteStream();
 ...
 pc.onconnectionstatechange=()=>{const st=pc.connectionState;callUiStatus(st==='connected'?'CONECTADO':st==='connecting'?'CONECTANDO…':st==='failed'?'FALHA NA CONEXÃO':st.toUpperCase());if(['failed','closed'].includes(st))setTimeout(()=>closeTeamMeeting(false),500)};
 pc.onicecandidate=async e=>{if(!e.candidate||!activeCallId)return;try{await setDoc(callDocRef(callId),{[role==='caller'?'callerCandidates':'calleeCandidates']:arrayUnion(rtcCandidate(e.candidate)),updatedAt:serverTimestamp()},{merge:true})}catch(err){console.warn('High Call ICE',err)}};
 return pc;
}
function watchActiveCall(callId,role){if(activeCallUnsubscribe)activeCallUnsubscribe();activeCallUnsubscribe=onSnapshot(callDocRef(callId),async snap=>{if(!snap.exists())return closeTeamMeeting(false);const d=snap.data();if(d.status==='ended'||d.status==='rejected')return closeTeamMeeting(false);const list=role==='caller'?(d.calleeCandidates||[]):(d.callerCandidates||[]);for(const c of list){const key=JSON.stringify(c);if(seenRemoteCandidates.has(key))continue;seenRemoteCandidates.add(key);try{await activePeer?.addIceCandidate(new RTCIceCandidate(c))}catch(e){console.warn('High Call candidato',e)}}if(role==='caller'&&d.answer&&!activePeer?.currentRemoteDescription){try{await activePeer.setRemoteDescription(new RTCSessionDescription(d.answer));callUiStatus('CONECTANDO…')}catch(e){console.warn('High Call resposta',e)}}});}
async function startTeamMeeting(mode='video'){
 if(!canEditModule('chat'))return permissionDeniedMessage('chat',true);if(!chatRecipientEmail)return alert('Selecione o usuário que deseja chamar.');
 try{const target=hmUser(chatRecipientEmail);showCallOverlay(`Chamada • ${target?hmUserName(target):chatRecipientEmail}`,mode,false);callUiStatus('PEDINDO MICROFONE…');await prepareLocalMedia(mode);const ref=doc(callCol);activeCallId=ref.id;const pc=createPeer(ref.id,'caller');const offer=await pc.createOffer();await pc.setLocalDescription(offer);await setDoc(ref,{caller:String(currentUser.email||'').toLowerCase(),callee:String(chatRecipientEmail||'').toLowerCase(),participants:chatParticipants(currentUser.email,chatRecipientEmail),mode,status:'ringing',offer:rtcDesc(pc.localDescription),callerCandidates:[],calleeCandidates:[],createdAt:serverTimestamp(),updatedAt:serverTimestamp()});watchActiveCall(ref.id,'caller');callUiStatus('CHAMANDO…');await addDoc(chatCol,{texto:'',callId:ref.id,callMode:mode,recipientEmail:chatRecipientEmail,conversationId:chatConversationId(currentUser.email,chatRecipientEmail),participants:chatParticipants(currentUser.email,chatRecipientEmail),email:currentUser.email||'',nome:currentProfile?.name||currentUser.displayName||currentUser.email,cargo:currentProfile?.cargo||currentProfile?.role||'',createdAt:serverTimestamp(),createdAtText:new Date().toISOString()});}catch(e){await closeTeamMeeting(false);alert('Não foi possível iniciar a chamada: '+e.message)}}
async function acceptIncomingCall(id,data){try{activeCallId=id;chatRecipientEmail=data.caller;showCallOverlay(`Chamada • ${hmUserName(hmUser(data.caller))}`,data.mode||'audio',false);callUiStatus('ACEITANDO…');await prepareLocalMedia(data.mode||'audio');const pc=createPeer(id,'callee');await pc.setRemoteDescription(new RTCSessionDescription(data.offer));const answer=await pc.createAnswer();await pc.setLocalDescription(answer);await setDoc(callDocRef(id),{answer:rtcDesc(pc.localDescription),status:'active',answeredAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});watchActiveCall(id,'callee');callUiStatus('CONECTANDO…')}catch(e){await setDoc(callDocRef(id),{status:'rejected',updatedAt:serverTimestamp()},{merge:true}).catch(()=>{});await closeTeamMeeting(false);alert('Não foi possível atender: '+e.message)}}
async function rejectIncomingCall(id){await setDoc(callDocRef(id),{status:'rejected',updatedAt:serverTimestamp()},{merge:true}).catch(()=>{});$('#incomingCallBar')?.classList.add('hidden')}
function startCallInbox(){if(callInboxUnsubscribe){callInboxUnsubscribe();callInboxUnsubscribe=null}if(!currentUser||!canViewModule('chat'))return;const me=String(currentUser.email||'').toLowerCase();callInboxUnsubscribe=onSnapshot(query(callCol,where('callee','==',me),limit(10)),snap=>{const ringing=snap.docs.map(x=>({id:x.id,...x.data()})).filter(x=>x.status==='ringing').sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0))[0];const bar=$('#incomingCallBar');if(!bar)return;if(!ringing||activeCallId){bar.classList.add('hidden');return}bar.classList.remove('hidden');$('#incomingCallName').textContent=`${hmUserName(hmUser(ringing.caller))} está chamando`;$('#incomingCallType').textContent=ringing.mode==='video'?'CHAMADA DE VÍDEO':'CHAMADA DE ÁUDIO';$('#incomingCallAccept').onclick=()=>{bar.classList.add('hidden');acceptIncomingCall(ringing.id,ringing)};$('#incomingCallReject').onclick=()=>rejectIncomingCall(ringing.id)});}
async function closeTeamMeeting(signal=true){const id=activeCallId;if(signal&&id)await setDoc(callDocRef(id),{status:'ended',endedAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true}).catch(()=>{});if(activeCallUnsubscribe){activeCallUnsubscribe();activeCallUnsubscribe=null}try{activePeer?.close()}catch{};activePeer=null;activeLocalStream?.getTracks().forEach(t=>t.stop());activeRemoteStream?.getTracks().forEach(t=>t.stop());activeLocalStream=null;activeRemoteStream=null;activeCallId='';seenRemoteCandidates.clear();const o=$('#teamMeetingOverlay');o?.classList.add('hidden');const l=$('#teamCallLocalVideo'),r=$('#teamCallRemoteVideo');if(l)l.srcObject=null;if(r)r.srcObject=null;callUiStatus('')}
function toggleCallMic(){const t=activeLocalStream?.getAudioTracks?.()[0];if(t){t.enabled=!t.enabled;setCallButtons()}}
function toggleCallCam(){const t=activeLocalStream?.getVideoTracks?.()[0];if(t){t.enabled=!t.enabled;setCallButtons()}}
...
$('#spotifySaveBtn')?.addEventListener('click',saveSpotifyConfig);$('#spotifySaveClientBtn')?.addEventListener('click',saveSpotifyClient);$('#spotifyLoginBtn')?.addEventListener('click',spotifyLogin);$('#spotifySearchBtn')?.addEventListener('click',spotifySearch);$('#spotifySearchInput')?.addEventListener('keydown',e=>{if(e.key==='Enter')spotifySearch()});$('#spotifyPlayBtn')?.addEventListener('click',()=>spotifyPlayer?.togglePlay());$('#spotifyPrevBtn')?.addEventListener('click',()=>spotifyPlayer?.previousTrack());$('#spotifyNextBtn')?.addEventListener('click',()=>spotifyPlayer?.nextTrack());$('#spotifyVolume')?.addEventListener('input',e=>{const v=Math.max(0,Math.min(100,Number(e.target.value)||0));const out=$('#spotifyVolumeValue');if(out)out.textContent=`${Math.round(v)}%`;localStorage.setItem('highos_spotify_volume',String(v));spotifyPlayer?.setVolume(v/100).catch?.(err=>console.warn('Spotify volume',err))});$('#spotifyProgress')?.addEventListener('change',async e=>{if(!spotifyPlayer||!spotifyMiniState?.duration)return;const ms=Math.round((Number(e.target.value)||0)/1000*spotifyMiniState.duration);try{await spotifyPlayer.seek(ms);spotifyMiniState={...spotifyMiniState,position:ms};spotifyUpdateProgress(ms,spotifyMiniState.duration)}catch(err){console.warn('Spotify seek',err)}});$('#spotifyDockMain')?.addEventListener('click',e=>{if(e.target.closest('#spotifyPlayGlyph')){e.stopPropagation();spotifyPlayer?.togglePlay();return}const dock=$('#spotifyTopDock');const open=!dock?.classList.contains('open');dock?.classList.toggle('open',open);e.currentTarget.setAttribute('aria-expanded',String(open))});$('#spotifyDockClose')?.addEventListener('click',()=>{const dock=$('#spotifyTopDock');dock?.classList.remove('open');$('#spotifyDockMain')?.setAttribute('aria-expanded','false')});$('#spotifyDockOpenPage')?.addEventListener('click',()=>{document.querySelector('[data-page="administracao"]')?.click();$('#spotifyTopDock')?.classList.remove('open')});document.querySelectorAll('[data-spotify-tab]').forEach(b=>b.addEventListener('click',()=>spotifySetTab(b.dataset.spotifyTab)));$('#spotifyRefreshPlaylists')?.addEventListener('click',()=>spotifyLoadPlaylists(true));document.addEventListener('click',e=>{const dock=$('#spotifyTopDock');if(dock?.classList.contains('open')&&!dock.contains(e.target)){dock.classList.remove('open');$('#spotifyDockMain')?.setAttribute('aria-expanded','false')}});
$('#chatRecipientSelect')?.addEventListener('change',e=>selectChatRecipient(e.target.value));$('#chatVideoCallBtn')?.addEventListener('click',()=>startTeamMeeting('video'));$('#chatAudioCallBtn')?.addEventListener('click',()=>startTeamMeeting('audio'));$('#teamChatLauncher')?.addEventListener('click',()=>toggleFloatingChat());$('#chatFloatMin')?.addEventListener('click',()=>toggleFloatingChat(false));$('#chatPageOpenFloat')?.addEventListener('click',()=>toggleFloatingChat(true));$('#floatingChatSendBtn')?.addEventListener('click',()=>sendChatMessage('#floatingChatInput'));$('#floatingChatInput')?.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendChatMessage('#floatingChatInput')}});$('#chatAttachInput')?.addEventListener('change',e=>{prepareChatAttachment(e.target.files?.[0]);e.target.value=''});$('#teamMeetingClose')?.addEventListener('click',()=>closeTeamMeeting(true));$('#teamCallMic')?.addEventListener('click',toggleCallMic);$('#teamCallCam')?.addEventListener('click',toggleCallCam);$('#hmContactSearch')?.addEventListener('input',renderHmContacts);$('#hmNewDmBtn')?.addEventListener('click',()=>{$('#hmContactSearch')?.focus()});$('#hmEmojiBtn')?.addEventListener('click',()=>toggleHmPicker(true,'emoji'));$('#hmStickerBtn')?.addEventListener('click',()=>toggleHmPicker(true,'sticker'));$('#hmGifBtn')?.addEventListener('click',()=>$('#hmGifInput')?.click());$('#hmGifInput')?.addEventListener('change',async e=>{const f=e.target.files?.[0];if(f){await prepareChatAttachment(f);await sendChatMessage('#floatingChatInput')}e.target.value=''});
...
function grParseCoord(v=''){
 ...
 return {x:nums[0],y:nums[1],z:nums[2],h:Number.isFinite(nums[3])?nums[3]:null};
}
function grParseRoute(text=''){return String(text).split(/\r?\n/).map((raw,line)=>({raw:raw.trim().replace(/^\d+\s*[-–:]\s*/,''),line:line+1})).filter(x=>x.raw).map(x=>({...x,p:grParseCoord(x.raw)}));}
...
function grCurrent(){const g=$('#fGroup')?.value||'';return faccoes.find(x=>x.group===g)||currentFactionFromForm()||{};}
...
function grRequestText(action='auto'){
 const f=grCurrent(),group=f.group||'',parsed=grParseRoute($('#grRouteInput')?.value||'').filter(x=>x.p),saved=grSavedPoints(f),hasSaved=saved.length>0;
 if(action==='delete')return ['Assunto: Remoção de rota de farm exclusiva','','Solicitação:','','- Remoção da rota de farm exclusiva',`- Group: "${group}"`,'',`Nome da Rota: RotaExclusiva${group}`,'',`- Solicito a remoção da rota de farm exclusiva atualmente vinculada ao Group "${group}".`,'',`- Permissão: "${group}"`].join('\n');
 const update=action==='update'||(action==='auto'&&hasSaved),subject=update?'Atualização de rota de farm exclusiva':'Ativação de rota de farm exclusiva';
 return [`Assunto: ${subject}`,'','Solicitação:','',`- ${subject}`,`- Group: "${group}"`,'','- Blips da rota nova:','',`Nome da Rota: RotaExclusiva${group}`,'',...parsed.map(x=>grFmtPoint(x.p)),'',`- Permissão: "${group}"`,...(update?['','- A nova rota deverá substituir integralmente a rota exclusiva atualmente vinculada ao Group.']:[])].join('\n');
}
function grShowRequest(action='auto'){const txt=grRequestText(action);$('#grRouteRequestText').value=txt;$('#grRouteRequestBox').classList.remove('hidden');return txt}
function grCollectStructures(f){const out=[],seen=new Set(),routeRaw=mergedTechProfile(f)?.rota?.pontos||'';const add=(label,v,kind='ESTRUTURA')=>{if(!v||typeof v!=='string'||v===routeRaw)return;const p=grParseCoord(v);if(!p)return;const key=`${p.x.toFixed(2)},${p.y.toFixed(2)},${p.z.toFixed(2)}`;if(seen.has(key))return;seen.add(key);out.push({label,p,kind})};
 add('QG / Local principal',f.cds,'QG');const b=f.beneficios||{},t=mergedTechProfile(f);const named=[['Craft',t.craft?.cds||b.craft],['Farm / Início',t.rota?.inicio||t.farm?.cds||b.farm],['Garagem Pública · Blip',b.garagemPublicaBlip],['Garagem Pública · Spawn',b.garagemPublicaSpawn],['Garagem VIP · Blip',b.garagemVipBlip],['Garagem VIP · Spawn',b.garagemVipSpawn],['Heliponto · Blip',b.helipontoBlip],['Heliponto · Spawn',b.helipontoSpawn],['Loja de Roupas',b.lojaRoupas],['Barbearia',b.barbearia],['Tatuagem',b.tatuagem],['Shop Exclusivo',b.shopExclusivo],['Arena',b.arena],['Telão',b.telaoCds]];named.forEach(([n,v])=>add(n,v));
 const walk=(obj,path='')=>{if(!obj||typeof obj!=='object')return;Object.entries(obj).forEach(([k,v])=>{const np=path?`${path} · ${k}`:k;if(typeof v==='string'&&/(-?\d+(?:\.\d+)?\s*,\s*){2}/.test(v))add(np,v);else if(v&&typeof v==='object'&&!Array.isArray(v))walk(v,np)})};walk(f.perfilOperacional||{},'Perfil');walk(t.estruturaExtra||{},'Extra');return out;}
function grEnsureMap(){if(grMap||!$('#grGroupMap')||!window.L)return;const crs=L.extend({},L.CRS.Simple,{projection:L.Projection.LonLat,scale:z=>Math.pow(2,z),zoom:s=>Math.log(s)/Math.LN2,distance:(a,b)=>Math.hypot(b.lng-a.lng,b.lat-a.lat),transformation:new L.Transformation(0.02072,117.3,-0.0205,172.8),infinite:true});grMap=L.map('grGroupMap',{crs,minZoom:0,maxZoom:5,zoomControl:true,attributionControl:false});L.tileLayer('https://cdn.jsdelivr.net/gh/Trusted-Studios/mapStyles@main/styleAtlas/{z}/{x}/{y}.jpg',{minZoom:0,maxZoom:5,noWrap:true,crossOrigin:'anonymous'}).addTo(grMap);grRouteLayer=L.layerGroup().addTo(grMap);grStructureLayer=L.layerGroup().addTo(grMap);grMap.setView(L.latLng(-500,500),2);}
function grRouteIcon(n){return L.divIcon({className:'',html:`<div class="gr-marker">${n}</div>`,iconSize:[28,28],iconAnchor:[14,14]})}
function grStructIcon(kind){return L.divIcon({className:'',html:`<div class="${kind==='QG'?'gr-qg-marker':'gr-structure-marker'}" style="width:18px;height:18px"></div>`,iconSize:[22,22],iconAnchor:[11,11]})}
function grRenderMap(){grEnsureMap();if(!grMap)return;grRouteLayer.clearLayers();grStructureLayer.clearLayers();const parsed=grParseRoute($('#grRouteInput')?.value||'').filter(x=>x.p),latlngs=[];if($('#grMapRoute')?.checked!==false){parsed.forEach((x,i)=>{const ll=L.latLng(x.p.y,x.p.x);latlngs.push(ll);L.marker(ll,{icon:grRouteIcon(i+1)}).bindPopup(`<b>Ponto ${String(i+1).padStart(2,'0')}</b><br>${esc(grFmtPoint(x.p))}`).addTo(grRouteLayer)});if(latlngs.length>1)L.polyline(latlngs,{weight:4,opacity:.9,dashArray:'8 5'}).addTo(grRouteLayer)}
 const structs=$('#grMapStructures')?.checked===false?[]:grCollectStructures(grCurrent());structs.forEach(x=>{const ll=L.latLng(x.p.y,x.p.x);latlngs.push(ll);L.marker(ll,{icon:grStructIcon(x.kind)}).bindPopup(`<b>${esc(x.label)}</b><br>${esc(grFmtPoint(x.p))}`).addTo(grStructureLayer)});$('#grMapLegend').innerHTML=`<span>● ${parsed.length} pontos da rota</span><span>◆ ${structs.filter(x=>x.kind==='QG').length} QG</span><span>● ${structs.filter(x=>x.kind!=='QG').length} outras CDS</span><span>Traçado atual: direto entre pontos</span>`;if(latlngs.length)try{grMap.fitBounds(L.latLngBounds(latlngs).pad(.15),{maxZoom:4})}catch{};setTimeout(()=>grMap.invalidateSize(),80);}
function grRenderRows(){const parsed=grParseRoute($('#grRouteInput')?.value||''),valid=parsed.filter(x=>x.p);$('#grRouteCount').textContent=String(valid.length);const bad=parsed.filter(x=>!x.p);$('#grRouteParseInfo').textContent=bad.length?`${valid.length} CDS válidas • ${bad.length} linha(s) não reconhecida(s): ${bad.map(x=>x.line).join(', ')}`:`${valid.length} CDS válidas • ordem preservada`;
 const box=$('#grRoutePointRows');if(!box)return;box.innerHTML=valid.length?valid.map((x,i)=>`<div class="gr-point-edit-row" data-gr-i="${i}"><span>${String(i+1).padStart(2,'0')}</span><input value="${esc(grFmtPoint(x.p).replace(/,$/,''))}"><button type="button" class="mini-btn danger" title="Remover ponto">×</button></div>`).join(''):'<div class="route-empty"><b>SEM CDS</b><span>Cole os pontos recebidos pelo takefarm.</span></div>';box.querySelectorAll('.gr-point-edit-row').forEach((row,i)=>{row.querySelector('input').onchange=()=>{const arr=grParseRoute($('#grRouteInput').value).filter(x=>x.p).map(x=>grFmtPoint(x.p));arr[i]=row.querySelector('input').value;$('#grRouteInput').value=arr.join('\n');grRouteDirty=true;grRenderRouteUi(false)};row.querySelector('button').onclick=()=>{const arr=grParseRoute($('#grRouteInput').value).filter(x=>x.p).map(x=>grFmtPoint(x.p));arr.splice(i,1);$('#grRouteInput').value=arr.join('\n');grRouteDirty=true;grRenderRouteUi(false)}})}
function grRenderRouteUi(loadSaved=true){const f=grCurrent(),t=mergedTechProfile(f),saved=routePointList(t.rota?.pontos||f?.beneficios?.rotaBlips||'');if(loadSaved&&!grRouteDirty)$('#grRouteInput').value=saved.join('\n');const group=f.group||'—',count=grParseRoute($('#grRouteInput').value).filter(x=>x.p).length,status=t.rota?.status||((f?.beneficios?.rotaExclusiva||saved.length)?'ATIVA':'ROTA_PADRAO');$('#grRouteGroup').textContent=group;$('#grRouteName').textContent=group==='—'?'—':`RotaExclusiva${group}`;$('#grRoutePermission').textContent=group;$('#grRouteStatus').textContent=status.replaceAll('_',' ');$('#grRouteStatus').classList.toggle('warn',status==='AGUARDANDO_REMOCAO');grRenderRows();grRenderMap();}
async function grSaveRoute(){const f=grCurrent(),group=f.group,pts=grParseRoute($('#grRouteInput').value).filter(x=>x.p);if(!group)return alert('Group não identificado.');if(!pts.length)return alert('Cole ao menos uma CDS válida.');const old=grSavedPoints(f),action=old.length?'update':'activate',text=grRequestText(action);const t=mergedTechProfile(f);t.rota={...(t.rota||{}),nome:`RotaExclusiva${group}`,pontos:pts.map(x=>grFmtPoint(x.p).replace(/,$/,'')).join('\n'),status:'ATIVA',origem:'TAKEFARM_MANUAL',atualizadoPor:currentUser.email,atualizadoEm:new Date().toISOString()};const benefits={...(f.beneficios||{}),rotaExclusiva:true,rotaBlips:t.rota.pontos};try{await setDoc(doc(db,'highos','data','faccoes',group),{perfilTecnico:t,beneficios:benefits,updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{sessionId:currentSessionId||'',tipo:old.length?'ROTA_EXCLUSIVA_ATUALIZADA':'ROTA_EXCLUSIVA_ATIVADA',group,descricao:`Rota exclusiva • ${pts.length} CDS`,antes:{pontos:old},depois:{pontos:pts.map(x=>x.p)},solicitacaoTexto:text,usuario:currentUser.email,data:serverTimestamp()});await archiveTechnicalRequest({tipo:'ROTA_FARM',titulo:old.length?'Atualização de rota de farm exclusiva':'Ativação de rota de farm exclusiva',texto:text},f,'ROTA_EXCLUSIVA');grRouteDirty=false;await loadFaccoes();const fresh=faccoes.find(x=>x.group===group);if(fresh){renderTechProfile(fresh);$('#fRotaExclusiva').checked=true}grShowRequest(old.length?'update':'activate');grRenderRouteUi(true);alert(`Rota de ${group} salva com ${pts.length} pontos. A solicitação foi gerada e arquivada.`)}catch(e){alert('Erro ao salvar rota: '+e.message)}}
async function grDeleteRoute(){const f=grCurrent(),group=f.group,old=grSavedPoints(f);if(!group||!old.length)return alert('Este Group não possui rota exclusiva cadastrada.');if(!confirm(`Gerar solicitação de remoção da rota exclusiva de ${group}?\n\nA rota ficará como AGUARDANDO REMOÇÃO e não será apagada até a confirmação final.`))return;const text=grRequestText('delete'),t=mergedTechProfile(f);t.rota={...(t.rota||{}),status:'AGUARDANDO_REMOCAO',remocaoSolicitadaEm:new Date().toISOString(),remocaoSolicitadaPor:currentUser.email};try{await setDoc(doc(db,'highos','data','faccoes',group),{perfilTecnico:t,updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{sessionId:currentSessionId||'',tipo:'ROTA_EXCLUSIVA_REMOCAO_SOLICITADA',group,descricao:'Remoção da rota exclusiva solicitada',solicitacaoTexto:text,usuario:currentUser.email,data:serverTimestamp()});await archiveTechnicalRequest({tipo:'ROTA_FARM',titulo:'Remoção de rota de farm exclusiva',texto:text},f,'ROTA_EXCLUSIVA');grShowRequest('delete');grRouteDirty=false;await loadFaccoes();grRenderRouteUi(true);alert('Solicitação de remoção gerada. As CDS foram preservadas até a remoção ser confirmada.')}catch(e){alert('Erro ao gerar remoção: '+e.message)}}
async function grMapPng(){grRenderMap();await new Promise(r=>setTimeout(r,500));if(!window.html2canvas)return alert('Captura de imagem indisponível.');try{const canvas=await html2canvas($('#grGroupMap'),{useCORS:true,backgroundColor:'#174f70'}),a=document.createElement('a');a.download=`mapa-rota-${grCurrent().group||'group'}.png`;a.href=canvas.toDataURL('image/png');a.click()}catch(e){alert('Não foi possível gerar a imagem: '+e.message)}}
$('#grRouteInput')?.addEventListener('input',()=>{grRouteDirty=true;grRenderRows()});$('#grRoutePreview')?.addEventListener('click',()=>{grRenderRows();grRenderMap()});$('#grRouteSave')?.addEventListener('click',grSaveRoute);$('#grRouteRequest')?.addEventListener('click',()=>grShowRequest('auto'));$('#grRouteDelete')?.addEventListener('click',grDeleteRoute);$('#grRouteCopyRequest')?.addEventListener('click',async()=>{try{await navigator.clipboard.writeText($('#grRouteRequestText').value);$('#grRouteCopyRequest').textContent='COPIADO ✓';setTimeout(()=>$('#grRouteCopyRequest').textContent='COPIAR',1200)}catch{}});$('#grRouteMapPng')?.addEventListener('click',grMapPng);$('#grMapRoute')?.addEventListener('change',grRenderMap);$('#grMapStructures')?.addEventListener('change',grRenderMap);
...
const _renderTechProfileV835=renderTechProfile;renderTechProfile=function(f){_renderTechProfileV835(f);grRouteDirty=false;if(document.querySelector('[data-tech-panel="rota-exclusiva"]')?.classList.contains('active'))setTimeout(()=>grRenderRouteUi(true),50)};
...
function v836RoutePoints(f){const t=mergedTechProfile(f),raw=t?.rota?.pontos||f?.beneficios?.rotaBlips||'';return routePointList(raw).map(grParseCoord).filter(Boolean)}
function v836RouteLabel(f){const t=mergedTechProfile(f),pts=v836RoutePoints(f);if(t?.rota?.status==='AGUARDANDO_REMOCAO'&&pts.length)return 'AGUARDANDO REMOÇÃO';return pts.length?'ROTA EXCLUSIVA':'ROTA PADRÃO'}
function v836CardImage(f){const url=String(f?.imagemAnuncio||'').trim();return url?`<div class="unified-card-image"><img src="${esc(url)}" alt="Arte ${esc(f.group||'Group')}" loading="lazy"></div>`:`<div class="unified-card-image empty"><span>HIGH</span><small>${esc(f?.group||'GROUP')}</small></div>`}
...
const _renderFaccoesV836=renderFaccoes;renderFaccoes=function(){_renderFaccoesV836();v836RenderOrganizations()};
function v836ApplyUnifiedUi(){
 ...
 const farm=document.querySelector('[data-tech-panel="farm"]');farm?.querySelectorAll('.v836-legacy-route,.legacy-route-toggle,.legacy-route-fields').forEach(x=>x.classList.add('hidden'));
 const status=$('#grRouteStatus');if(status&&/SEM ROTA/i.test(status.textContent||''))status.textContent='ROTA PADRÃO';
}
...
async function grConfirmRouteRemovalV836(){
 const f=grCurrent(),group=f?.group,old=grSavedPoints(f);if(!group||!old.length)return alert('Este Group já está usando a Rota Padrão.');
 const t=mergedTechProfile(f);if(t?.rota?.status!=='AGUARDANDO_REMOCAO'&&!confirm(`Confirmar que a Rota Exclusiva de ${group} já foi removida na cidade?`))return;
 ...
 t.rota={...(t.rota||{}),nome:'',pontos:'',status:'ROTA_PADRAO',remocaoConfirmadaEm:new Date().toISOString(),remocaoConfirmadaPor:currentUser.email};
 const benefits={...(f.beneficios||{}),rotaExclusiva:false,rotaBlips:''};
 try{await setDoc(doc(db,'highos','data','faccoes',group),{perfilTecnico:t,beneficios,updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{sessionId:currentSessionId||'',tipo:'ROTA_EXCLUSIVA_REMOVIDA',group,descricao:'Rota exclusiva removida; Group retornou automaticamente para Rota Padrão',antes:{pontos:old},depois:{tipo:'ROTA_PADRAO'},usuario:currentUser.email,data:serverTimestamp()});grRouteDirty=false;await loadFaccoes();const fresh=faccoes.find(x=>x.group===group);if(fresh)renderTechProfile(fresh);grRenderRouteUi(true);alert(`${group} agora utiliza ROTA PADRÃO.`)}catch(e){alert('Erro ao confirmar remoção: '+e.message)}
}
$('#grRouteConfirmDelete')?.addEventListener('click',grConfirmRouteRemovalV836);

const _grRenderRouteUiV836=grRenderRouteUi;grRenderRouteUi=function(loadSaved=true){_grRenderRouteUiV836(loadSaved);const f=grCurrent(),t=mergedTechProfile(f),pts=grSavedPoints(f),status=t?.rota?.status;const badge=$('#grRouteStatus'),confirmBtn=$('#grRouteConfirmDelete');if(badge){badge.textContent=status==='AGUARDANDO_REMOCAO'&&pts.length?'AGUARDANDO REMOÇÃO':pts.length?'ROTA EXCLUSIVA':'ROTA PADRÃO';badge.classList.toggle('warn',status==='AGUARDANDO_REMOCAO')}if(confirmBtn)confirmBtn.classList.toggle('hidden',!(status==='AGUARDANDO_REMOCAO'&&pts.length));};
...
let gsMap=null,gsLayer=null,gsRouteLayer=null,gsFilter='TODOS';
function gsRows(){if(!techDraft)techDraft=mergedTechProfile(grCurrent()||{});if(!Array.isArray(techDraft.estruturaCatalogo))techDraft.estruturaCatalogo=[];return techDraft.estruturaCatalogo}
function gsCoord(v){return grParseCoord(String(v||''))}
function gsIcon(type){const glyph={CRAFT:'C',FARM:'F',LOJA:'L','BAÚ':'B','RÁDIO':'R',AMENIDADE:'A',GARAGEM:'G',QG:'Q',OUTRO:'•'}[type]||'•';return L.divIcon({className:'',html:`<div class="gs-map-marker">${glyph}</div>`,iconSize:[24,24],iconAnchor:[12,12]})}
function gsEnsureMap(){if(gsMap||!$('#gsMap')||!window.L)return;const crs=L.extend({},L.CRS.Simple,{projection:L.Projection.LonLat,scale:z=>Math.pow(2,z),zoom:s=>Math.log(s)/Math.LN2,distance:(a,b)=>Math.hypot(b.lng-a.lng,b.lat-a.lat),transformation:new L.Transformation(0.02072,117.3,-0.0205,172.8),infinite:true});gsMap=L.map('gsMap',{crs,minZoom:0,maxZoom:5,zoomControl:true,attributionControl:false});L.tileLayer('https://cdn.jsdelivr.net/gh/Trusted-Studios/mapStyles@main/styleAtlas/{z}/{x}/{y}.jpg',{minZoom:0,maxZoom:5,noWrap:true,crossOrigin:'anonymous'}).addTo(gsMap);gsLayer=L.layerGroup().addTo(gsMap);gsRouteLayer=L.layerGroup().addTo(gsMap);gsMap.setView(L.latLng(-500,500),2)}
function gsRenderMap(fit=false){gsEnsureMap();if(!gsMap)return;gsLayer.clearLayers();gsRouteLayer.clearLayers();const bounds=[];const visible=gsRows().filter(x=>gsFilter==='TODOS'||x.tipo===gsFilter);visible.forEach((x,i)=>{const p=gsCoord(x.cds);if(!p)return;const ll=L.latLng(p.y,p.x);bounds.push(ll);L.marker(ll,{icon:gsIcon(x.tipo)}).bindPopup(`<b>${esc(x.nome||x.tipo)}</b><br>${esc(x.tipo)}<br>${esc(x.cds||'')}<br><button onclick="navigator.clipboard?.writeText('${String(x.cds||'').replaceAll("'","\\'")}')">Copiar CDS</button>`).addTo(gsLayer)});if($('#gsShowRoute')?.checked){const pts=grSavedPoints(grCurrent());const ls=pts.map(p=>L.latLng(p.y,p.x));ls.forEach((ll,i)=>{bounds.push(ll);L.marker(ll,{icon:grRouteIcon(i+1)}).bindPopup(`<b>Rota Exclusiva • ${i+1}</b>`).addTo(gsRouteLayer)});if(ls.length>1)L.polyline(ls,{weight:4,opacity:.85,dashArray:'8 5'}).addTo(gsRouteLayer)};const withCds=visible.filter(x=>gsCoord(x.cds)).length;$('#gsLegend').innerHTML=`<span>${withCds} estruturas no mapa</span><span>${visible.length-withCds} sem CDS</span><span>${$('#gsShowRoute')?.checked?grSavedPoints(grCurrent()).length:0} pontos de rota</span>`;if((fit||!gsMap._gsFitted)&&bounds.length){try{gsMap.fitBounds(L.latLngBounds(bounds).pad(.15),{maxZoom:4});gsMap._gsFitted=true}catch{}}setTimeout(()=>gsMap.invalidateSize(),80)}
function gsRender(load=true){if(!$('#gsList'))return;const f=grCurrent();if(load&&techDraft&&!Array.isArray(techDraft.estruturaCatalogo))techDraft.estruturaCatalogo=[];const rows=gsRows();$('#gsCount').textContent=`${rows.length} ITENS`;$('#gsImportArmas01')?.classList.toggle('hidden',String(f?.group||'').toUpperCase()!=='ARMAS01'||rows.length>0);const counts=Object.fromEntries(GS_TYPES.map(t=>[t,rows.filter(x=>x.tipo===t).length]));$('#gsFilters').innerHTML=['TODOS',...GS_TYPES].map(t=>`<button type="button" class="gs-filter ${gsFilter===t?'active':''}" data-gsf="${t}">${t}${t==='TODOS'?` · ${rows.length}`:counts[t]?` · ${counts[t]}`:''}</button>`).join('');const shown=rows.map((x,i)=>({...x,_i:i})).filter(x=>gsFilter==='TODOS'||x.tipo===gsFilter);$('#gsList').innerHTML=shown.length?shown.map(x=>`<div class="gs-row" data-gsi="${x._i}"><select class="gs-type admin-only">${GS_TYPES.map(t=>`<option ${x.tipo===t?'selected':''}>${t}</option>`).join('')}</select><input class="gs-name" value="${esc(x.nome||'')}" placeholder="Nome / identificação"><input class="gs-coord" value="${esc(x.cds||'')}" placeholder="x,y,z,h • pode ficar vazio para rádio"><button type="button" class="mini-btn danger admin-only gs-del">×</button></div>`).join(''):'<div class="gs-empty">Nenhuma estrutura neste filtro.</div>';$('#gsFilters').querySelectorAll('[data-gsf]').forEach(b=>b.onclick=()=>{gsFilter=b.dataset.gsf;gsRender(false)});$('#gsList').querySelectorAll('[data-gsi]').forEach(row=>{const i=+row.dataset.gsi;const sync=()=>{const r=gsRows()[i];r.tipo=row.querySelector('.gs-type')?.value||r.tipo;r.nome=row.querySelector('.gs-name')?.value.trim()||'';r.cds=row.querySelector('.gs-coord')?.value.trim()||'';gsRenderMap(false)};row.querySelectorAll('input,select').forEach(el=>el.onchange=sync);row.querySelector('.gs-del')?.addEventListener('click',()=>{gsRows().splice(i,1);gsRender(false)})});gsRenderMap(load)}
function gsAdd(){gsRows().push({tipo:'OUTRO',nome:'',cds:''});gsFilter='TODOS';gsRender(false);setTimeout(()=>$('#gsList .gs-row:last-child .gs-name')?.focus(),30)}
async function gsSave(){const f=grCurrent(),group=f?.group;if(!group)return;getTechProfileFromForm();techDraft.estruturaCatalogo=gsRows();try{await setDoc(doc(db,'highos','data','faccoes',group),{perfilTecnico:clonePlain(techDraft),updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{sessionId:currentSessionId||'',tipo:'ESTRUTURA_GROUP_ATUALIZADA',group,descricao:`Estrutura administrativa atualizada • ${gsRows().length} itens`,usuario:currentUser.email,data:serverTimestamp()});const local=faccoes.find(x=>x.group===group);if(local)local.perfilTecnico=clonePlain(techDraft);alert(`Estrutura de ${group} salva com ${gsRows().length} itens.`)}catch(e){alert('Erro ao salvar estrutura: '+e.message)}}
function gsImportArmas01(){if(String(grCurrent()?.group||'').toUpperCase()!=='ARMAS01')return;const seed=[
 {tipo:'CRAFT',nome:'Bancada Armas01',cds:'1149.2, -99.9, 61.4'},
 {tipo:'FARM',nome:'Rota padrão ilegal • Ponto 1',cds:'1264.5, -179.1, 106.4'},{tipo:'FARM',nome:'Rota padrão ilegal • Ponto 2',cds:'2666.1, 2434.5, 55.5'},{tipo:'FARM',nome:'Rota padrão ilegal • Ponto 3',cds:'-1431.4, 2306.5, 30.8'},{tipo:'FARM',nome:'Rota padrão ilegal • Ponto 4',cds:'-2121.3, 2482.0, 9.9'},
 {tipo:'LOJA',nome:'LojaExclusivaArmas01',cds:'1154.5, -136.7, 61.1'},
 {tipo:'BAÚ',nome:'Armas01 • Líder',cds:'1146.9, -112.1, 64.3'},{tipo:'BAÚ',nome:'Armas01Trofeu',cds:'1273.0, -231.7, 99.7'},{tipo:'BAÚ',nome:'Geras Armas01',cds:'-2082.7, -1014.6, 5.9'},
 {tipo:'RÁDIO',nome:'Frequência 123',cds:''},{tipo:'RÁDIO',nome:'Frequência 244',cds:''},
 {tipo:'AMENIDADE',nome:'Barbearia',cds:'1205.3, -187.4, 68.4'},{tipo:'AMENIDADE',nome:'Loja de Roupas',cds:'1201.9, -181.0, 68.4'},{tipo:'AMENIDADE',nome:'Telão',cds:'1247.2, -166.2, 103.8'},{tipo:'AMENIDADE',nome:'Arena',cds:'1286.4, -275.5, 99.7'},
 {tipo:'GARAGEM',nome:'Garagem VIP',cds:'1121.8, -148.9, 60.8'},{tipo:'GARAGEM',nome:'Blindado',cds:'1243.4, -63.5, 69.4'},{tipo:'GARAGEM',nome:'Garagem',cds:'1145.7, -128.0, 60.8'}];techDraft.estruturaCatalogo=seed;gsRender(false);alert('Dados legíveis do /fac do Armas01 carregados. Confira e clique SALVAR ESTRUTURA. A rota exclusiva continua sendo administrada na aba Rota Exclusiva.')}
$('#gsAdd')?.addEventListener('click',gsAdd);$('#gsSave')?.addEventListener('click',gsSave);$('#gsImportArmas01')?.addEventListener('click',gsImportArmas01);$('#gsFit')?.addEventListener('click',()=>gsRenderMap(true));$('#gsShowRoute')?.addEventListener('change',()=>gsRenderMap(false));

// Imagem do card: reaproveita todos os registros antigos conhecidos de divulgação antes de cair no placeholder.
function v8361CardImageUrl(f){const candidates=[f?.imagemAnuncio,f?.anuncioDiscordStatus?.imagemUrl,f?.imagemUrl,f?.imagem,f?.foto,f?.cardImagem,f?.arteDiscord];return String(candidates.find(x=>typeof x==='string'&&x.trim())||'').trim()}
v836CardImage=function(f){const url=v8361CardImageUrl(f);return url?`<div class="unified-card-image"><img src="${esc(url)}" alt="Arte ${esc(f.group||'Group')}" loading="lazy" referrerpolicy="no-referrer"><a class="card-image-open" href="${esc(url)}" target="_blank" rel="noopener" onclick="event.stopPropagation()">ABRIR ARTE</a></div>`:`<div class="unified-card-image empty"><span>HIGH</span><small>${esc(f?.group||'GROUP')}</small></div>`}
...
const GS_TYPES_V8362=['QG','CRAFT','FARM','LOJA','BAÚ','RÁDIO','AMENIDADE','GARAGEM','HELIPONTO','BLINDADO','TELÃO','OUTRO'];
...
function gsLegacyCatalog(f={}){
 const t=mergedTechProfile(f),o=t.operacional||opBlank(),out=[];
 const add=(tipo,nome,cds='',secondary='',detalhe='')=>{if(!gsNorm(cds)&&!gsNorm(secondary)&&!detalhe)return;out.push({tipo,nome,cds:gsNorm(cds),secondary:gsNorm(secondary),detalhe,status:'ATIVO',origem:'LEGADO'})};
 ...
 add('LOJA','Loja da Facção',o.lojaFac?.cds);add('AMENIDADE','Bar',o.bar?.cds);add('AMENIDADE','Barbearia',o.barbearia?.cds);add('AMENIDADE','Tatuagem',o.tatuagem?.cds);add('AMENIDADE','Loja de Roupas',o.roupas?.cds);add('AMENIDADE','Arena',o.arena?.cds);add('AMENIDADE','Uniforme',o.uniforme?.local,'',o.uniforme?.arquivo||'');
 (o.garagens||[]).forEach((g,i)=>add('GARAGEM',({PUBLICA:'Garagem Pública 1',PUBLICA_2:'Garagem Pública 2',FACCAO:'Garagem Facção / VIP',SERVICO:'Garagem Serviço / VIP Org'}[g.tipo]||`Garagem ${i+1}`),g.blip,g.spawn,g.veiculos||''));
 (o.helipontos||[]).forEach((h,i)=>add('HELIPONTO',`Heliponto ${i+1}`,h.blip,h.spawn));
 add('BLINDADO','Garagem de Blindados',o.blindados?.blip,o.blindados?.spawn,[o.blindados?.vagas?`Vagas: ${o.blindados.vagas}`:'',o.blindados?.veiculos?`Veículo: ${o.blindados.veiculos}`:''].filter(Boolean).join(' • '));
 const te=o.telao||{};if(te.ativo||te.cds||te.postit){add('TELÃO',te.modelo||'Telão',te.cds,te.postit,[te.tipo,te.permissao].filter(Boolean).join(' • '));(te.sons||[]).filter(Boolean).forEach((x,i)=>add('TELÃO',`Telão • Som ${i+1}`,x,'','Ponto de som'));}
 return out;
}
function gsMergeLegacy(f,existing=[]){
 const rows=(existing||[]).map(x=>({...x,status:x.status||'ATIVO'})),keys=new Set();
 const key=x=>[x.tipo,gsNorm(x.nome).toLowerCase(),gsNorm(x.cds),gsNorm(x.secondary)].join('|');
 rows.forEach(x=>keys.add(key(x)));
 gsLegacyCatalog(f).forEach(x=>{const k=key(x);if(!keys.has(k)){rows.push(x);keys.add(k)}});return rows;
}
function gsMigrateCurrent(show=true){const f=grCurrent();if(!f)return;const before=gsRows().length;techDraft.estruturaCatalogo=gsMergeLegacy(f,gsRows());gsFilter='TODOS';gsRender(false);if(show)alert(`${techDraft.estruturaCatalogo.length-before} registro(s) antigo(s) consolidados. Confira o mapa e clique SALVAR ESTRUTURA.`)}
...
const _gsRenderV8362=gsRender;gsRender=function(load=true){if(load&&grCurrent()){if(!techDraft)techDraft=mergedTechProfile(grCurrent());techDraft.estruturaCatalogo=gsMergeLegacy(grCurrent(),techDraft.estruturaCatalogo||[])}return _gsRenderV8362(load)};

function gsRequestText(row,action,oldRow=null){const group=grCurrent()?.group||'{Group}',kind=row?.tipo||'Estrutura',name=row?.nome||kind;const fmt=x=>x?`{ ${gsNorm(x)} }`:'—';if(action==='NOVO')return `Assunto: Adição de ${kind}\n\nSolicitação:\n\n- Adicionar ${name} ao Group ${group}.\n- CDS / Blip: ${fmt(row.cds)}\n- Spawn / CDS secundária: ${fmt(row.secondary)}\n- Permissão: ${group}`;if(action==='REMOVER')return `Assunto: Remoção de ${kind}\n\nSolicitação:\n\n- Remover ${name} do Group ${group}.\n- CDS / Blip atual: ${fmt(row.cds)}\n- Spawn / CDS secundária atual: ${fmt(row.secondary)}\n- Permissão: ${group}`;return `Assunto: Alteração de ${kind}\n\nSolicitação:\n\n- Alterar ${name} do Group ${group}.\n\nConfiguração atual:\n- CDS / Blip: ${fmt(oldRow?.cds)}\n- Spawn / CDS secundária: ${fmt(oldRow?.secondary)}\n\nNova configuração:\n- CDS / Blip: ${fmt(row.cds)}\n- Spawn / CDS secundária: ${fmt(row.secondary)}\n\n- Permissão: ${group}`}
window.gsCopyRequest=function(i,action='ALTERAR'){const r=gsRows()[i];if(!r)return;const txt=gsRequestText(r,action,r._original||r);navigator.clipboard?.writeText(txt);alert('Solicitação gerada e copiada para a área de transferência.')}
window.gsMapEdit=function(i){gsFilter='TODOS';gsRender(false);setTimeout(()=>{const el=document.querySelector(`#gsList [data-gsi="${i}"]`);el?.scrollIntoView({behavior:'smooth',block:'center'});el?.querySelector('.gs-name')?.focus()},50)}
window.gsMapRemove=function(i){const r=gsRows()[i];if(!r||!confirm(`Gerar remoção de ${r.nome||r.tipo}? O ponto continuará registrado até a execução ser confirmada.`))return;r.status='REMOCAO_PENDENTE';navigator.clipboard?.writeText(gsRequestText(r,'REMOVER'));gsRender(false);alert('Remoção marcada como pendente e solicitação copiada. O cadastro não foi apagado.')}
...
gsRender=function(load=true){
 if(!$('#gsList'))return;const f=grCurrent();if(load&&f){if(!techDraft)techDraft=mergedTechProfile(f);techDraft.estruturaCatalogo=gsMergeLegacy(f,techDraft.estruturaCatalogo||[])}const rows=gsRows();$('#gsCount').textContent=`${rows.length} ITENS`;
 const counts=Object.fromEntries(GS_TYPES.map(t=>[t,rows.filter(x=>x.tipo===t).length]));$('#gsFilters').innerHTML=['TODOS',...GS_TYPES].map(t=>`<button type="button" class="gs-filter ${gsFilter===t?'active':''}" data-gsf="${t}">${t}${t==='TODOS'?` · ${rows.length}`:counts[t]?` · ${counts[t]}`:''}</button>`).join('');
 const shown=rows.map((x,i)=>({...x,_i:i})).filter(x=>gsFilter==='TODOS'||x.tipo===gsFilter);$('#gsList').innerHTML=shown.length?shown.map(x=>`<div class="gs-row" data-gsi="${x._i}"><select class="gs-type admin-only">${GS_TYPES.map(t=>`<option ${x.tipo===t?'selected':''}>${t}</option>`).join('')}</select><input class="gs-name" value="${esc(x.nome||'')}" placeholder="Nome / identificação"><input class="gs-coord" value="${esc(x.cds||'')}" placeholder="CDS principal / Blip"><input class="gs-secondary" value="${esc(x.secondary||'')}" placeholder="Spawn / CDS secundária"><span class="gs-status ${esc(x.status||'ATIVO')}">${esc((x.status||'ATIVO').replaceAll('_',' '))}</span><button type="button" class="mini-btn admin-only gs-req" title="Gerar solicitação">DC</button><button type="button" class="mini-btn danger admin-only gs-del" title="Solicitar remoção">×</button></div>`).join(''):'<div class="gs-empty">Nenhuma estrutura neste filtro.</div>';
 $('#gsFilters').querySelectorAll('[data-gsf]').forEach(b=>b.onclick=()=>{gsFilter=b.dataset.gsf;gsRender(false)});$('#gsList').querySelectorAll('[data-gsi]').forEach(row=>{const i=+row.dataset.gsi;const r=gsRows()[i];const sync=()=>{if(!r._original)r._original={...r};r.tipo=row.querySelector('.gs-type')?.value||r.tipo;r.nome=row.querySelector('.gs-name')?.value.trim()||'';r.cds=row.querySelector('.gs-coord')?.value.trim()||'';r.secondary=row.querySelector('.gs-secondary')?.value.trim()||'';if(r.status==='ATIVO')r.status=r.origem==='NOVO'?'PENDENTE':'ALTERACAO_PENDENTE';gsRenderMap(false)};row.querySelectorAll('input,select').forEach(el=>el.onchange=sync);row.querySelector('.gs-req')?.addEventListener('click',()=>window.gsCopyRequest(i,r.status==='PENDENTE'?'NOVO':r.status==='REMOCAO_PENDENTE'?'REMOVER':'ALTERAR'));row.querySelector('.gs-del')?.addEventListener('click',()=>window.gsMapRemove(i))});gsRenderMap(load)
};
gsAdd=function(){gsRows().push({tipo:'OUTRO',nome:'',cds:'',secondary:'',detalhe:'',status:'PENDENTE',origem:'NOVO'});gsFilter='TODOS';gsRender(false);setTimeout(()=>$('#gsList .gs-row:last-child .gs-name')?.focus(),30)};
$('#gsAdd')?.replaceWith($('#gsAdd').cloneNode(true));$('#gsAdd')?.addEventListener('click',gsAdd);
...
let gsView='LISTA';
function gsCleanRow(r){const x={...r};delete x._original;delete x._i;return x}
async function gsPersist(eventType='ESTRUTURA_ATUALIZADA',description='Estrutura atualizada'){
 const f=grCurrent(),group=f?.group;if(!group)return false;try{techDraft.estruturaCatalogo=gsRows().map(gsCleanRow);await setDoc(doc(db,'highos','data','faccoes',group),{perfilTecnico:clonePlain(techDraft),updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{sessionId:currentSessionId||'',tipo:eventType,group,descricao:description,usuario:currentUser.email,data:serverTimestamp()});const local=faccoes.find(x=>x.group===group);if(local)local.perfilTecnico=clonePlain(techDraft);return true}catch(e){alert('Erro ao salvar: '+e.message);return false}
}
function gsStatusText(s){return ({ATIVO:'ATIVO',PENDENTE:'NOVA • AGUARDANDO',ALTERACAO_PENDENTE:'ALTERAÇÃO PENDENTE',REMOCAO_PENDENTE:'REMOÇÃO PENDENTE'})[s]||'ATIVO'}
function gsCardCoord(label,v){return v?`<div><small>${label}</small><code>${esc(v)}</code></div>`:''}
function gsRenderKpis(rows){const active=rows.filter(x=>(x.status||'ATIVO')==='ATIVO').length,pending=rows.filter(x=>x.status&&x.status!=='ATIVO').length,points=rows.reduce((n,x)=>n+(gsCoord(x.cds)?1:0)+(gsCoord(x.secondary)?1:0),0);$('#gsKpis').innerHTML=`<div class="gs-kpi"><span>Estruturas</span><b>${rows.length}</b></div><div class="gs-kpi"><span>Ativas</span><b>${active}</b></div><div class="gs-kpi"><span>Pendências</span><b>${pending}</b></div><div class="gs-kpi"><span>Pontos no mapa</span><b>${points}</b></div>`;$('#gsCount').textContent=`${rows.length} ESTRUTURAS`}
function gsSetView(v){gsView=v;$('#gsLayout')?.classList.toggle('v837-list',v==='LISTA');$('#gsLayout')?.classList.toggle('v837-map',v==='MAPA');$('#gsViewList')?.classList.toggle('active',v==='LISTA');$('#gsViewMap')?.classList.toggle('active',v==='MAPA');if(v==='MAPA')setTimeout(()=>{gsRenderMap(true);gsMap?.invalidateSize()},80)}
function gsModalHtml(row={},i=-1,isNew=false){const status=row.status||'ATIVO';return `<div class="gs-modal-backdrop" id="gsModal"><div class="gs-modal"><div class="gs-modal-head"><div><div class="eyebrow">${isNew?'NOVA ESTRUTURA':'EDITAR ESTRUTURA'}</div><h3>${esc(row.nome||'Estrutura')}</h3></div><button class="mini-btn" id="gsmClose">✕</button></div><div class="gs-modal-grid"><label>TIPO<select id="gsmType">${GS_TYPES.map(t=>`<option ${row.tipo===t?'selected':''}>${t}</option>`).join('')}</select></label><label>NOME<input id="gsmName" value="${esc(row.nome||'')}"></label><label>CDS PRINCIPAL / BLIP<input id="gsmCds" value="${esc(row.cds||'')}" placeholder="x,y,z,h"></label><label>SPAWN / CDS SECUNDÁRIA<input id="gsmSecondary" value="${esc(row.secondary||'')}" placeholder="x,y,z,h"></label></div><div class="gs-choice"><button id="gsmCurrent" class="active">ATUALIZAR BASE<br><small>Já existe assim na cidade</small></button><button id="gsmPlan">PLANEJAR ALTERAÇÃO<br><small>Quero solicitar mudança</small></button></div><div class="gs-modal-actions"><button class="btn-secondary compact" id="gsmCancel">CANCELAR</button><button class="btn-primary compact" id="gsmSave">SALVAR COMO CONFIGURAÇÃO ATUAL</button></div></div></div>`}
function gsOpenEditor(i=-1){const isNew=i<0,row=isNew?{tipo:'OUTRO',nome:'',cds:'',secondary:'',status:'ATIVO'}:{...gsRows()[i]};document.body.insertAdjacentHTML('beforeend',gsModalHtml(row,i,isNew));let mode='CURRENT';const modal=$('#gsModal'),save=$('#gsmSave');const setMode=m=>{mode=m;$('#gsmCurrent').classList.toggle('active',m==='CURRENT');$('#gsmPlan').classList.toggle('active',m==='PLAN');save.textContent=m==='CURRENT'?'SALVAR COMO CONFIGURAÇÃO ATUAL':isNew?'GERAR SOLICITAÇÃO DE CRIAÇÃO':'GERAR SOLICITAÇÃO DE ALTERAÇÃO'};$('#gsmCurrent').onclick=()=>setMode('CURRENT');$('#gsmPlan').onclick=()=>setMode('PLAN');const close=()=>modal.remove();$('#gsmClose').onclick=close;$('#gsmCancel').onclick=close;save.onclick=async()=>{const n={tipo:$('#gsmType').value,nome:$('#gsmName').value.trim(),cds:$('#gsmCds').value.trim(),secondary:$('#gsmSecondary').value.trim()};if(!n.nome)return alert('Informe o nome da estrutura.');if(mode==='CURRENT'){if(isNew)gsRows().push({...n,status:'ATIVO',origem:'BASE'});else Object.assign(gsRows()[i],n,{status:'ATIVO',pending:null,origem:'BASE'});const ok=await gsPersist('ESTRUTURA_BASE_CONFIRMADA',`${n.tipo} ${n.nome} confirmado como configuração atual`);if(ok){close();gsRender(false)}}else{if(isNew){gsRows().push({...n,status:'PENDENTE',origem:'NOVO'});i=gsRows().length-1;navigator.clipboard?.writeText(gsRequestText(gsRows()[i],'NOVO'))}else{const r=gsRows()[i];r.pending=n;r.status='ALTERACAO_PENDENTE';navigator.clipboard?.writeText(gsRequestText(n,'ALTERAR',r))}const ok=await gsPersist('ESTRUTURA_SOLICITACAO_GERADA',`${n.tipo} ${n.nome} aguardando execução`);if(ok){close();gsRender(false);alert('Solicitação copiada. A configuração atual foi preservada até você confirmar a execução.')}}}}
window.gsConfirmPending=async function(i){const r=gsRows()[i];if(!r)return;if(r.status==='PENDENTE'){r.status='ATIVO';r.origem='BASE'}else if(r.status==='ALTERACAO_PENDENTE'&&r.pending){Object.assign(r,r.pending);r.pending=null;r.status='ATIVO';r.origem='BASE'}else if(r.status==='REMOCAO_PENDENTE'){const name=r.nome;gsRows().splice(i,1);const ok=await gsPersist('ESTRUTURA_REMOCAO_CONFIRMADA',`${name} removido após confirmação de execução`);if(ok)gsRender(false);return}const ok=await gsPersist('ESTRUTURA_EXECUCAO_CONFIRMADA',`${r.nome||r.tipo} confirmado como executado na cidade`);if(ok)gsRender(false)}
window.gsCancelPending=async function(i){const r=gsRows()[i];if(!r)return;if(r.status==='PENDENTE'){gsRows().splice(i,1)}else{r.pending=null;r.status='ATIVO'};const ok=await gsPersist('ESTRUTURA_PENDENCIA_CANCELADA','Pendência de estrutura cancelada');if(ok)gsRender(false)}
window.gsRequestRemoval=async function(i){const r=gsRows()[i];if(!r||!confirm(`Solicitar remoção de ${r.nome||r.tipo}?`))return;r.status='REMOCAO_PENDENTE';navigator.clipboard?.writeText(gsRequestText(r,'REMOVER'));const ok=await gsPersist('ESTRUTURA_REMOCAO_SOLICITADA',`Remoção solicitada: ${r.nome||r.tipo}`);if(ok){gsRender(false);alert('Solicitação de remoção copiada. O ponto continuará ativo na base até confirmar a execução.')}}
window.gsCopyRowCds=function(i){const r=gsRows()[i];navigator.clipboard?.writeText([r?.cds,r?.secondary,r?.postit,...(r?.speakers||[])].filter(Boolean).join('\n'))}

gsRender=function(load=true){if(!$('#gsList'))return;const f=grCurrent();if(load&&f){if(!techDraft)techDraft=mergedTechProfile(f);techDraft.estruturaCatalogo=gsMergeLegacy(f,techDraft.estruturaCatalogo||[])}const rows=gsRows();gsRenderKpis(rows);const counts=Object.fromEntries(GS_TYPES.map(t=>[t,rows.filter(x=>x.tipo===t).length]));$('#gsFilters').innerHTML=['TODOS',...GS_TYPES].map(t=>`<button type="button" class="gs-filter ${gsFilter===t?'active':''}" data-gsf="${t}">${t}${t==='TODOS'?` · ${rows.length}`:counts[t]?` · ${counts[t]}`:''}</button>`).join('');const shown=rows.map((x,i)=>({...x,_i:i})).filter(x=>gsFilter==='TODOS'||x.tipo===gsFilter);$('#gsList').innerHTML=shown.length?shown.map(x=>`<article class="gs-card"><div class="gs-card-head"><div><div class="gs-card-type">${esc(x.tipo)}</div><h4>${esc(x.nome||'Sem nome')}</h4></div><span class="gs-status-pill ${esc(x.status||'ATIVO')}">${gsStatusText(x.status)}</span></div><div class="gs-card-cds">${gsCardCoord('CDS PRINCIPAL / BLIP',x.cds)}${gsCardCoord('SPAWN / SECUNDÁRIA',x.secondary)}${!x.cds&&!x.secondary?'<div><small>COORDENADAS</small><code>Sem CDS cadastrada</code></div>':''}</div>${x.status==='ALTERACAO_PENDENTE'&&x.pending?`<div class="gs-card-pending"><b>NOVA CONFIGURAÇÃO</b><br>${esc(x.pending.cds||'—')}${x.pending.secondary?`<br>${esc(x.pending.secondary)}`:''}</div>`:''}<div class="gs-card-actions"><button class="mini-btn" onclick="gsSetView('MAPA');setTimeout(()=>gsRenderMap(true),60)">VER MAPA</button><button class="mini-btn" onclick="gsCopyRowCds(${x._i})">COPIAR CDS</button><button class="mini-btn admin-only" onclick="gsOpenEditor(${x._i})">EDITAR</button>${(x.status||'ATIVO')==='ATIVO'?`<button class="mini-btn danger admin-only" onclick="gsRequestRemoval(${x._i})">REMOVER</button>`:`<button class="mini-btn admin-only" onclick="gsConfirmPending(${x._i})">CONFIRMAR EXECUÇÃO</button><button class="mini-btn danger admin-only" onclick="gsCancelPending(${x._i})">CANCELAR</button>`}</div></article>`).join(''):'<div class="gs-empty-v837">Nenhuma estrutura neste filtro.</div>';$('#gsFilters').querySelectorAll('[data-gsf]').forEach(b=>b.onclick=()=>{gsFilter=b.dataset.gsf;gsRender(false)});gsSetView(gsView);gsRenderMap(load)};
window.gsOpenEditor=gsOpenEditor;window.gsSetView=gsSetView;
...
let v9StructureOriginal=[],v9StructureDirty=false;
const v9Clone=v=>JSON.parse(JSON.stringify(v||[]));
function v9CleanRows(rows){return (rows||[]).map(r=>({tipo:r.tipo||'OUTRO',nome:r.nome||'',cds:r.cds||'',secondary:r.secondary||'',radio:r.radio||'',postit:r.postit||'',speakers:Array.isArray(r.speakers)?r.speakers.filter(Boolean).slice(0,4):[]}));}
function v9MarkDirty(){v9StructureDirty=true;const bar=$('#gsDirtyBar');bar?.classList.remove('hidden');const a=v9CleanRows(v9StructureOriginal),b=v9CleanRows(gsRows());let changes=0;const max=Math.max(a.length,b.length);for(let i=0;i<max;i++)if(JSON.stringify(a[i]||null)!==JSON.stringify(b[i]||null))changes++;if($('#gsDirtyText'))$('#gsDirtyText').textContent=`${changes||1} alteração${changes===1?'':'ões'} não salva${changes===1?'':'s'}`;}
function v9QGCds(){const q=gsRows().find(r=>r.tipo==='QG'&&gsCoord(r.cds));return q?.cds||''}
function v9Diff(oldRows,newRows){const key=r=>`${r.tipo}|${r.nome}`.toLowerCase(),om=new Map(oldRows.map(r=>[key(r),r])),nm=new Map(newRows.map(r=>[key(r),r])),out=[];for(const [k,o] of om)if(!nm.has(k))out.push({acao:'REMOVER',antes:o});for(const [k,n] of nm){const o=om.get(k);if(!o)out.push({acao:'ADICIONAR',depois:n});else if(JSON.stringify(v9CleanRows([o])[0])!==JSON.stringify(v9CleanRows([n])[0]))out.push({acao:'ALTERAR',antes:o,depois:n})}return out}
function v9StructureRequest(group,diff){
 ...
 const lines=[`Assunto: Alterações estruturais - ${group}`,'','Solicitação:','',`- Favor realizar as seguintes alterações no Group "${group}":`,''];
 diff.forEach((d,i)=>{
...
gsOpenEditor=function(i=-1){
 const isNew=i<0,row=isNew?{tipo:'OUTRO',nome:'',cds:'',secondary:'',radio:'',postit:'',speakers:[]}:{...gsRows()[i]};
 document.body.insertAdjacentHTML('beforeend',v9EditorHtml(row,isNew));
 const close=()=>$('#gsModal')?.remove();$('#gsmClose').onclick=close;$('#gsmCancel').onclick=close;
 ...
 $('#gsmType').onchange=syncType;syncType();
 ...
 $('#gsmSave').onclick=()=>{const tipo=$('#gsmType').value;const n={tipo,nome:$('#gsmName').value.trim(),cds:$('#gsmCds').value.trim(),secondary:tipo==='TELÃO'?'':($('#gsmSecondary')?.value.trim()||''),radio:tipo==='RÁDIO'?($('#gsmRadio')?.value.trim()||''):'',postit:tipo==='TELÃO'?($('#gsmPostit')?.value.trim()||''):'',speakers:tipo==='TELÃO'?[...document.querySelectorAll('.gsmSpeaker')].map(x=>x.value.trim()).filter(Boolean).slice(0,4):[]};if(!n.nome)return alert('Informe o nome/identificação.');if(isNew)gsRows().push(n);else gsRows()[i]=n;v9MarkDirty();close();gsRender(false)}
};
window.gsOpenEditor=gsOpenEditor;
window.gsRequestRemoval=function(i){const r=gsRows()[i];if(!r)return;if(!confirm(`Excluir ${r.nome||r.tipo} da estrutura do Group?`))return;gsRows().splice(i,1);v9MarkDirty();gsRender(false)};
window.gsDeleteSimple=window.gsRequestRemoval;
window.gsCopyRowCds=function(i){const r=gsRows()[i];navigator.clipboard?.writeText([r?.cds,r?.secondary,r?.postit,...(r?.speakers||[])].filter(Boolean).join('\n'))}
const v9OldRender=gsRender;
gsRender=function(load=true){if(load){const f=grCurrent();if(f){if(!techDraft)techDraft=mergedTechProfile(f);/* V9 não consolida legado automaticamente */if(!Array.isArray(techDraft.estruturaCatalogo))techDraft.estruturaCatalogo=[];v9StructureOriginal=v9Clone(techDraft.estruturaCatalogo);v9StructureDirty=false;$('#gsDirtyBar')?.classList.add('hidden')}}if(!$('#gsList'))return;const rows=gsRows();$('#gsCount').textContent=`${rows.length} ITENS`;const counts=Object.fromEntries(GS_TYPES.map(t=>[t,rows.filter(x=>x.tipo===t).length]));$('#gsFilters').innerHTML=['TODOS',...GS_TYPES].map(t=>`<button type="button" class="gs-filter ${gsFilter===t?'active':''}" data-gsf="${t}">${t==='TODOS'?'TODOS':t}${t==='TODOS'?` ${rows.length}`:counts[t]?` ${counts[t]}`:''}</button>`).join('');const shown=rows.map((x,i)=>({...x,_i:i})).filter(x=>gsFilter==='TODOS'||x.tipo===gsFilter);$('#gsList').innerHTML=shown.length?shown.map(x=>`<article class="gs-card"><div class="gs-card-head"><div><div class="gs-card-type">${esc(x.tipo)}</div><h4>${esc(x.nome||'Sem nome')}</h4></div></div><div class="gs-card-cds">${x.tipo==='RÁDIO'&&x.radio?`<div><small>FREQUÊNCIA</small><code>📡 ${esc(x.radio)}</code></div>`:''}${gsCardCoord('CDS PRINCIPAL / BLIP',x.cds)}${x.tipo==='TELÃO'?gsCardCoord('CDS POST-IT',x.postit):gsCardCoord('SPAWN / SECUNDÁRIA',x.secondary)}${x.tipo==='TELÃO'?(x.speakers||[]).map((c,j)=>gsCardCoord(`CAIXA DE SOM ${j+1}`,c)).join(''):''}${!x.cds&&!x.secondary&&x.tipo!=='RÁDIO'?'<div><small>COORDENADAS</small><code>Sem CDS</code></div>':''}</div><div class="gs-card-actions"><button type="button" class="mini-btn" onclick="gsFocusMap(event,${x._i})">VER NO MAPA</button><button class="mini-btn" onclick="gsCopyRowCds(${x._i})">COPIAR</button><button class="mini-btn admin-only" onclick="gsOpenEditor(${x._i})">EDITAR</button><button class="mini-btn danger admin-only" onclick="gsDeleteSimple(${x._i})">EXCLUIR</button></div></article>`).join(''):'<div class="gs-empty-v837">Nenhuma estrutura cadastrada neste filtro.</div>';$('#gsFilters').querySelectorAll('[data-gsf]').forEach(b=>b.onclick=()=>{gsFilter=b.dataset.gsf;gsRender(false)});gsSetView(gsView);gsRenderMap(load)};
const v9OldSetView=gsSetView;gsSetView=function(v){gsView=v;const layout=$('#gsLayout');if(!layout)return;layout.classList.toggle('v9-map-only',v==='MAPA');$('#gsViewList')?.classList.toggle('active',v!=='MAPA');$('#gsViewMap')?.classList.toggle('active',v==='MAPA');if(v==='MAPA')setTimeout(()=>{gsRenderMap(true);gsMap?.invalidateSize()},80)};window.gsSetView=gsSetView;
...
function mgmtStart(days,offset=0){const d=new Date();d.setHours(0,0,0,0);d.setDate(d.getDate()-offset-days+1);return d}
...
function mgmtPeriodStats(group,days,offset=0){const start=mgmtStart(days,offset),end=new Date();end.setHours(23,59,59,999);if(offset){end.setDate(end.getDate()-offset)}const rows=mgmtRowsFor(group,start,end),vals=rows.flatMap(r=>Object.values(metricSlots(r)).filter(Number.isFinite));const avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;const peak=vals.length?Math.max(...vals):0;const dates=new Set(rows.map(r=>mgmtDayKey(metricDateValue(r))));const expectedSlots=rows.length*4,actualSlots=rows.reduce((n,r)=>n+['14H','16H','21H','23H'].filter(h=>Number.isFinite(metricSlots(r)[h])).length,0);return {rows,avg,peak,days:dates.size,coverage:expectedSlots?actualSlots/expectedSlots*100:0,start,end}}
function mgmtPct(cur,prev){return prev>0?(cur-prev)/prev*100:(cur>0?null:0)}
function mgmtTrendText(v){if(v===null||!Number.isFinite(v))return 'base anterior zerada';return `${v>=0?'+':''}${v.toFixed(0)}%`}
function mgmtMedian(a){const x=a.filter(Number.isFinite).sort((a,b)=>a-b);if(!x.length)return 0;const m=Math.floor(x.length/2);return x.length%2?x[m]:(x[m-1]+x[m])/2}
function mgmtBuild(){
 const occupied=faccoes.filter(f=>f.status==='ATIVA'&&String(f.faccao||'').trim());
 const base=occupied.map(f=>{const w=mgmtPeriodStats(f.group,7),wp=mgmtPeriodStats(f.group,7,7),m=mgmtPeriodStats(f.group,30),mp=mgmtPeriodStats(f.group,30,30);return {f,w,wp,m,mp,wTrend:mgmtPct(w.avg,wp.avg),mTrend:mgmtPct(m.avg,mp.avg)}});
 const medBySeg={};[...new Set(base.map(x=>x.f.segmento||'OUTROS'))].forEach(seg=>{medBySeg[seg]=mgmtMedian(base.filter(x=>(x.f.segmento||'OUTROS')===seg&&x.w.days>=2&&x.w.avg>0).map(x=>x.w.avg))});
 const globalMed=mgmtMedian(base.filter(x=>x.w.avg>0).map(x=>x.w.avg));
 return base.map(x=>{const med=medBySeg[x.f.segmento||'OUTROS']||globalMed||1,rel=x.w.avg/med;let status='SAUDAVEL',reasons=[],score=0;
  const enough=x.w.days>=3&&x.w.coverage>=60,zeroWithData=x.w.coverage>=80&&x.w.days>=3&&x.w.avg===0;
  ...
  return {...x,status,reasons,score,segmentMedian:med,relative:rel};
 })
}
...
function orgV92Status(f={}){if(orgV92HasOccupant(f))return 'ASSUMIDA';return orgV92HasQG(f)?'DISPONIVEL':'INDISPONIVEL'}
...
function orgV92Segment(f={}){
 const raw=String(f.segmento||'').trim();if(raw)return raw;
 const g=String(f.group||'').toLowerCase();
 if(g.startsWith('armas'))return 'ARMAS';if(g.startsWith('municao')||g.startsWith('munição'))return 'MUNIÇÃO';if(g.startsWith('lavagem'))return 'LAVAGEM';if(g.startsWith('drogas'))return 'DROGAS';if(g.startsWith('desmanche'))return 'DESMANCHE';if(g.startsWith('contrabando'))return 'CONTRABANDO';if(g.includes('ilegalmec')||g.includes('ilegalmedic'))return 'APOIO';return 'OUTROS';
}
function orgV92Rows(){return faccoes.filter(f=>!f.removido).map(f=>({...f,__orgStatus:orgV92Status(f),__orgSegment:orgV92Segment(f)}))}
function orgV92Audit(rows=[]){
 const norm=s=>String(s||'').toLowerCase().replace(/[^a-z0-9]/g,'');const seen=new Map();
 rows.forEach(f=>{const k=norm(f.group);if(k)seen.set(k,(seen.get(k)||0)+1)});
 const duplicate=[...seen.values()].filter(n=>n>1).length;
 return {duplicate,assumedNoDate:rows.filter(f=>f.__orgStatus==='ASSUMIDA'&&!String(f.dataEntrega||'').trim()).length,assumedNoLeader:rows.filter(f=>f.__orgStatus==='ASSUMIDA'&&!String(f.lider||'').trim()).length,assumedNoCds:rows.filter(f=>f.__orgStatus==='ASSUMIDA'&&!String(f.cds||'').trim()).length,availableNoCds:rows.filter(f=>f.__orgStatus==='DISPONIVEL'&&!String(f.cds||'').trim()).length};
}
function orgV92Filtered(){
 const all=orgV92Rows(),q=String($('#facSearch')?.value||'').trim().toLowerCase(),seg=$('#facSegment')?.value||'',st=$('#facStatus')?.value||'';
...
renderFaccoes=function(){
 ...
 const st=$('#facStatus')?.value||'',seg=$('#facSegment')?.value||'';if($('#orgV92ListTitle'))$('#orgV92ListTitle').textContent=[st?orgV92StatusLabel(st):'TODAS AS ORGANIZAÇÕES',seg].filter(Boolean).join(' • ');if($('#facStats'))$('#facStats').textContent=`${rows.length} exibida(s) de ${all.length}`;
 const box=$('#facList');if(!box)return;if(!rows.length){box.innerHTML='<div class="placeholder"><b>⌕</b><h3>NENHUMA ORGANIZAÇÃO NESTE FILTRO</h3><p>Ajuste segmento, situação ou busca.</p></div>';return}
 ...
 const audit=orgV92Audit(all);box.insertAdjacentHTML('afterend',`<div id="orgV92Audit" class="org-v92-audit"><div class="org-v92-audit-head"><b>QUALIDADE CADASTRAL</b><span>Alertas administrativos — não alteram o status operacional</span></div><div class="org-v92-audit-items"><span class="${audit.duplicate?'danger':''}">${audit.duplicate} Group(s) duplicado(s)</span><span class="${audit.assumedNoDate?'warn':''}">${audit.assumedNoDate} assumida(s) sem data</span><span class="${audit.assumedNoLeader?'warn':''}">${audit.assumedNoLeader} assumida(s) sem líder</span><span class="${audit.assumedNoCds?'warn':''}">${audit.assumedNoCds} assumida(s) sem CDS</span><span class="${audit.availableNoCds?'warn':''}">${audit.availableNoCds} disponível(is) sem CDS</span></div></div>`);
 document.querySelectorAll('#orgV92Audit').forEach((el,i)=>{if(i<document.querySelectorAll('#orgV92Audit').length-1)el.remove()});
 box.querySelectorAll('tbody tr').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;openFac(r.dataset.orgV92Id)});box.querySelectorAll('.org-v92-open').forEach(b=>b.onclick=e=>{e.stopPropagation();openFac(b.dataset.id)});box.querySelectorAll('.org-v92-request').forEach(b=>b.onclick=e=>{e.stopPropagation();openRequestModal('',b.dataset.group)});box.querySelectorAll('.org-v92-deliver').forEach(b=>b.onclick=e=>{e.stopPropagation();openNewDelivery(b.dataset.group)});
}
function orgV92ReportRows(type='GERAL'){const all=orgV92Rows();return type==='GERAL'?all:all.filter(f=>f.__orgStatus===type)}
...
function orgV92OpenReport(){
 const type=$('#orgV92ReportType')?.value||'GERAL',rows=orgV92ReportRows(type).sort((a,b)=>String(a.__orgSegment).localeCompare(String(b.__orgSegment),'pt-BR')||String(a.group).localeCompare(String(b.group),'pt-BR',{numeric:true})),now=new Date(),c=s=>orgV92Rows().filter(f=>f.__orgStatus===s).length;
 ...
 const w=window.open('','_blank');if(!w)return alert('O navegador bloqueou a abertura do relatório. Libere pop-ups para o High OS.');w.document.open();w.document.write(html);w.document.close();
}
...
let missionCloudTimer=null,missionCloudBusy=false;
async function pullMissionsFromCloud(){
 ...
 try{
  ...
  return {missions:data.missions,updatedAtText:data.updatedAtText||'',updatedBy:data.updatedBy||''};
...
console.info('HIGH OS V9.5.3 · sistema carregado');
