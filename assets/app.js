...
// ===== HIGH OS V6.4 · PERFIS OFICIAIS DOS GROUPS · 07/09/2026 =====
...
const GROUP_PROFILE_SOURCE_VERSION='07/09/2026';
...
function sourceToGroupPatch(f,src){
 const oldB={...(f?.beneficios||{})}, oldT=mergedTechProfile(f||{}), b={...oldB}, t=clonePlain(oldT)||{};
 ...
 const patch={beneficios:b,perfilTecnico:t,perfilFonte:{...src,versao:GROUP_PROFILE_SOURCE_VERSION}};
...
async function updateOfficialGroupProfiles(){
 ...
 const entries=Object.entries(GROUP_PROFILE_SOURCE);if(!entries.length)return alert('Nenhum perfil oficial carregado.');
...
function defaultTechProfile(f={}){
 ...
 return {craft:{cds:b.craft||'',nome:f.segmento==='ARMAS'?'Armas de Pequeno, Médio e Grande Calibre':'',receitas:base},farm:{cds:b.farm||'',itens:[]},rota:{nome:b.rotaExclusiva?`RotaExclusiva${f.group||''}`:'',inicio:'',pontos:b.rotaBlips||''},estruturaExtra:{}};
}
function mergedTechProfile(f={}){
 ...
 return {craft:{cds:p?.craft?.cds??d.craft.cds,nome:p?.craft?.nome??d.craft.nome,receitas:(explicit?p.craft.receitas:d.craft.receitas).map(recipeNormalize)},farm:{cds:p?.farm?.cds??d.farm.cds,itens:Array.isArray(p?.farm?.itens)?p.farm.itens.map(x=>({...x})):[]},rota:{nome:p?.rota?.nome??d.rota.nome,inicio:p?.rota?.inicio??d.rota.inicio,pontos:p?.rota?.pontos??d.rota.pontos},estruturaExtra:{...(d.estruturaExtra||{}),...(p?.estruturaExtra||{})}};
}
...
function techAutoRequests(f=currentFactionFromForm()){
 ...
 if(JSON.stringify(oldT.farm)!==JSON.stringify(now.farm)||JSON.stringify(oldT.rota)!==JSON.stringify(now.rota)){
  const pts=String(now.rota?.pontos||'').split(/\r?\n|·/).map(x=>x.trim()).filter(Boolean),routeStart=String(now.rota?.inicio||'').trim(),items=(now.farm?.itens||[]).map(x=>`- ${x.nome||x.spawn}${x.spawn?` (${x.spawn})`:''}${x.qtd?` — ${x.qtd}`:''}`);
...
const _alvesAnswerV63Base=alvesAnswer;alvesAnswer=function(question=''){
 const q=alvesNorm(question),g=alvesFindGroup(question);if(g){const t=mergedTechProfile(g);
  ...
  if(q.includes('rota'))return {text:`Rota de ${g.group}:\n${t.rota?.nome?`Nome: ${t.rota.nome}\n`:''}${t.rota?.inicio?`Início: ${t.rota.inicio}\n`:''}${t.rota?.pontos?`Pontos: ${t.rota.pontos}`:'Nenhum ponto adicional de rota cadastrado.'}`,refs:[g.group,'Perfil Técnico','Rota']};
 }
