...
let currentUser=null,currentProfile=null,faccoes=[],solicitacoes=[],usuarios=[],organizacoes=[],crafts=[];
const facCol=collection(db,'highos','data','faccoes'), histCol=collection(db,'highos','data','historico'), reqCol=collection(db,'highos','data','solicitacoes'), deliveryCol=collection(db,'highos','data','entregas'), orgCol=collection(db,'highos','data','organizacoes'), craftCol=collection(db,'highos','data','crafts'), usersCol=collection(db,'users');
...
// ===== HIGH OS V6.3 · CRAFTS / RECEITAS VISUAIS =====
const CRAFT_BASE=[
 {id:'t54',nome:'T54',spawn:'WEAPON_PISTOL_MK2',imagem:'t54.png',segmento:'ARMAS',quantidade:1,nivel:5,maximo:10,status:'ATIVA',groups:['Armas01','Armas02','Armas04','Armas06','Armas07','Armas09','Armas10','Armas11'],ingredientes:[{nome:'Corpo de Pistola',spawn:'pistolbody',quantidade:22},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:12},{nome:'Dólar',spawn:'dollar',quantidade:8500}],observacoes:'Receita confirmada no catálogo operacional do High Ilegal OS.'},
 {id:'tec9',nome:'Tec-9',spawn:'WEAPON_MACHINEPISTOL',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Sub',spawn:'smgbody',quantidade:25},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:19},{nome:'Dólar',spawn:'dollar',quantidade:26000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'deagle',nome:'Deagle',spawn:'WEAPON_PISTOL50',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Pistola',spawn:'pistolbody',quantidade:16},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:13},{nome:'Dólar',spawn:'dollar',quantidade:18000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'m1922',nome:'M1922',spawn:'WEAPON_VINTAGEPISTOL',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Pistola',spawn:'pistolbody',quantidade:11},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:9},{nome:'Dólar',spawn:'dollar',quantidade:12000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'f2000',nome:'F2000',spawn:'WEAPON_ASSAULTSMG',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Sub',spawn:'smgbody',quantidade:30},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:24},{nome:'Dólar',spawn:'dollar',quantidade:31000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'ak102',nome:'AK-102',spawn:'WEAPON_ASSAULTRIFLE_MK2',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Rifle',spawn:'riflebody',quantidade:40},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:34},{nome:'Dólar',spawn:'dollar',quantidade:46000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'ak74n',nome:'AK-74N',spawn:'WEAPON_ASSAULTRIFLE',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Rifle',spawn:'riflebody',quantidade:35},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:30},{nome:'Dólar',spawn:'dollar',quantidade:70000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'fnfal',nome:'FN L1A1',spawn:'WEAPON_FNFAL',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Rifle',spawn:'riflebody',quantidade:38},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:31},{nome:'Dólar',spawn:'dollar',quantidade:70000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'sigsauer556',nome:'Sig Sauer 556',spawn:'WEAPON_SPECIALCARBINE_MK2',segmento:'ARMAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Corpo de Rifle',spawn:'riflebody',quantidade:47},{nome:'Chapa de Metal',spawn:'sheetmetal',quantidade:33},{nome:'Dólar',spawn:'dollar',quantidade:70000}],observacoes:'Receita-base do eixo de Armas.'},
 {id:'weedsack',nome:'Saco de Maconha',spawn:'weedsack',segmento:'DROGAS',quantidade:1,status:'ATIVA',ingredientes:[{nome:'Ziplock',spawn:'ziplock',quantidade:50},{nome:'Bud de Maconha',spawn:'weedbud',quantidade:1}],observacoes:'Receita confirmada para bancada de drogas.'}
];
function craftKey(v=''){return alvesNorm(v).replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||('craft-'+Date.now())}
function itemImage(spawn='',imagem=''){let f=String(imagem||'').trim()||String(spawn||'').trim()+'.png';return 'assets/itens/'+encodeURIComponent(f).replace(/%2F/gi,'/')}
function craftMerged(){const m=new Map(CRAFT_BASE.map(x=>[x.id,{...x,builtin:true}]));crafts.forEach(x=>m.set(x.id,{...m.get(x.id),...x,builtin:false}));return [...m.values()]}
function craftFmtQty(v){const n=Number(v);return Number.isFinite(n)?n.toLocaleString('pt-BR'):String(v||'—')}
function craftFind(text=''){const n=alvesNorm(text);return craftMerged().filter(c=>n.includes(alvesNorm(c.nome))||n.includes(alvesNorm(c.spawn))||alvesNorm(c.nome).includes(n)).sort((a,b)=>alvesNorm(b.nome).length-alvesNorm(a.nome).length)[0]||null}
async function loadCrafts(){try{const qs=await getDocs(craftCol);crafts=qs.docs.map(d=>({id:d.id,...d.data()}));renderCrafts()}catch(e){crafts=[];renderCrafts(e)}}
function renderCrafts(err){const grid=$('#craftGrid');if(!grid)return;const all=craftMerged(),q=alvesNorm($('#craftSearch')?.value||''),seg=$('#craftSegment')?.value||'',st=$('#craftStatus')?.value||'';const filtered=all.filter(c=>(!seg||c.segmento===seg)&&(!st||(c.status||'ATIVA')===st)&&(!q||alvesNorm([c.nome,c.spawn,c.segmento,(c.groups||[]).join(' '),(c.ingredientes||[]).map(i=>i.nome+' '+i.spawn).join(' ')].join(' ')).includes(q)));const active=all.filter(c=>(c.status||'ATIVA')==='ATIVA').length,segments=new Set(all.map(c=>c.segmento).filter(Boolean)).size,insumes=new Set(all.flatMap(c=>(c.ingredientes||[]).map(i=>i.spawn))).size;
 $('#craftStats').innerHTML=`<span><b>${all.length}</b> RECEITAS</span><span><b>${active}</b> ATIVAS</span><span><b>${segments}</b> SEGMENTOS</span><span><b>${filtered.length}</b> EXIBIDAS</span>`;
 $('#craftOverview').innerHTML=`<div class="craft-kpi"><span>CATÁLOGO VISUAL</span><b>${all.length}</b><small>receitas operacionais</small></div><div class="craft-kpi"><span>INSUMOS MAPEADOS</span><b>${insumes}</b><small>spawns diferentes</small></div><div class="craft-kpi"><span>BIBLIOTECA DE IMAGENS</span><b>assets/itens</b><small>imagem pelo spawn do item</small></div><div class="craft-kpi purple"><span>ALVESINHO</span><b>READY</b><small>consulta receitas e ingredientes</small></div>`;
 if(err&&!all.length){grid.innerHTML=`<div class="placeholder"><h3>ERRO AO CARREGAR</h3><p>${esc(err.message)}</p></div>`;return}if(!filtered.length){grid.innerHTML='<div class="placeholder"><b>⚒</b><h3>NENHUMA RECEITA</h3><p>Ajuste os filtros ou cadastre uma nova receita.</p></div>';return}
 grid.innerHTML=filtered.map(c=>`<article class="craft-card" data-id="${esc(c.id)}"><div class="craft-card-art"><img src="${itemImage(c.spawn,c.imagem)}" onerror="this.onerror=null;this.src='assets/high_logo.png'" alt="${esc(c.nome)}"><span class="status-chip ${(c.status||'ATIVA')==='ATIVA'?'ativa':'inativa'}">${esc(c.status||'ATIVA')}</span></div><div class="craft-card-body"><div class="craft-card-title"><div><span>${esc(c.segmento||'OUTROS')}</span><h3>${esc(c.nome)}</h3><code>${esc(c.spawn)}</code></div><b>x${craftFmtQty(c.quantidade||1)}</b></div><div class="craft-ingredient-strip">${(c.ingredientes||[]).slice(0,5).map(i=>`<div title="${esc(i.nome)}"><img src="${itemImage(i.spawn,i.imagem)}" onerror="this.onerror=null;this.src='assets/high_logo.png'" alt=""><span>x${craftFmtQty(i.quantidade)}</span></div>`).join('')}${(c.ingredientes||[]).length>5?`<div class="craft-more">+${(c.ingredientes||[]).length-5}</div>`:''}</div><div class="craft-meta"><span>${(c.ingredientes||[]).length} ingredientes</span>${c.nivel?`<span>Nível ${esc(c.nivel)}</span>`:''}${c.maximo?`<span>Máx. ${esc(c.maximo)}</span>`:''}</div><button class="mini-btn craft-open">VER RECEITA</button></div></article>`).join('');
 grid.querySelectorAll('.craft-card').forEach(el=>el.addEventListener('click',e=>{if(e.target.closest('button')||e.currentTarget===e.target){}openCraft(el.dataset.id)}));}
function craftIngredientRow(i={}){const row=document.createElement('div');row.className='craft-ing-edit';row.innerHTML=`<div class="craft-ing-image"><img src="${itemImage(i.spawn,i.imagem)}" onerror="this.onerror=null;this.src='assets/high_logo.png'" alt=""></div><label>Nome<input class="ci-name" value="${esc(i.nome||'')}"></label><label>Spawn<input class="ci-spawn" value="${esc(i.spawn||'')}"></label><label>Qtd<input class="ci-qty" type="number" min="0" step="1" value="${esc(i.quantidade??1)}"></label><button type="button" class="craft-remove">×</button>`;row.querySelector('.ci-spawn').addEventListener('input',e=>{row.querySelector('img').src=itemImage(e.target.value)});row.querySelector('.craft-remove').onclick=()=>row.remove();return row}
function addCraftIngredient(i={}){$('#craftIngredientsEditor')?.appendChild(craftIngredientRow(i))}
function openCraft(id=''){const c=id?craftMerged().find(x=>x.id===id):null;$('#craftId').value=c?.id||'';$('#craftModalTitle').textContent=c?.nome||'NOVA RECEITA';$('#craftName').value=c?.nome||'';$('#craftSpawn').value=c?.spawn||'';$('#craftImage').value=c?.imagem||'';$('#craftSeg').value=c?.segmento||'ARMAS';$('#craftOutputQty').value=c?.quantidade||1;$('#craftLevel').value=c?.nivel||'';$('#craftMax').value=c?.maximo||'';$('#craftState').value=c?.status||'ATIVA';$('#craftGroups').value=(c?.groups||[]).join(', ');$('#craftNotes').value=c?.observacoes||'';const ed=$('#craftIngredientsEditor');ed.innerHTML='';(c?.ingredientes||[{nome:'',spawn:'',quantidade:1}]).forEach(addCraftIngredient);updateCraftPreview();const admin=String(currentProfile?.role||'').toUpperCase()==='ADMIN';$('#craftForm').querySelectorAll('input,textarea,select').forEach(x=>x.disabled=!admin);$('#addIngredientBtn').style.display=admin?'inline-flex':'none';ed.querySelectorAll('.craft-remove').forEach(x=>x.style.display=admin?'grid':'none');$('#craftForm button[type="submit"]').style.display=admin?'inline-flex':'none';$('#craftModal').classList.remove('hidden')}
function updateCraftPreview(){const n=$('#craftName')?.value||'NOVO ITEM',sp=$('#craftSpawn')?.value||'spawn',im=$('#craftImage')?.value||'';$('#craftPreviewName').textContent=n;$('#craftPreviewSpawn').textContent=sp;const img=$('#craftPreviewImg');img.src=itemImage(sp,im);img.onerror=()=>{img.onerror=null;img.src='assets/high_logo.png'}}
function getCraftForm(){return {nome:$('#craftName').value.trim(),spawn:$('#craftSpawn').value.trim(),imagem:$('#craftImage').value.trim(),segmento:$('#craftSeg').value,quantidade:Number($('#craftOutputQty').value||1),nivel:Number($('#craftLevel').value||0)||'',maximo:Number($('#craftMax').value||0)||'',status:$('#craftState').value,groups:$('#craftGroups').value.split(',').map(x=>x.trim()).filter(Boolean),observacoes:$('#craftNotes').value.trim(),ingredientes:[...document.querySelectorAll('#craftIngredientsEditor .craft-ing-edit')].map(r=>({nome:r.querySelector('.ci-name').value.trim(),spawn:r.querySelector('.ci-spawn').value.trim(),quantidade:Number(r.querySelector('.ci-qty').value||0)})).filter(i=>i.nome||i.spawn)}}
function craftText(c){return `${c.nome} (${c.spawn})\nProduz: x${craftFmtQty(c.quantidade||1)}${c.nivel?`\nNível: ${c.nivel}`:''}${c.maximo?` • Máx.: ${c.maximo}`:''}\n\nReceita / insumos:\n${(c.ingredientes||[]).map(i=>`- ${i.nome||i.spawn} x${craftFmtQty(i.quantidade)}`).join('\n')||'- Nenhum ingrediente cadastrado'}${c.groups?.length?`\n\nGroups: ${c.groups.join(', ')}`:''}`}
$('#craftSearch')?.addEventListener('input',renderCrafts);$('#craftSegment')?.addEventListener('change',renderCrafts);$('#craftStatus')?.addEventListener('change',renderCrafts);$('#newCraftBtn')?.addEventListener('click',()=>openCraft());$('#craftModalClose')?.addEventListener('click',()=>$('#craftModal').classList.add('hidden'));$('#craftModal')?.addEventListener('click',e=>{if(e.target.id==='craftModal')e.currentTarget.classList.add('hidden')});$('#addIngredientBtn')?.addEventListener('click',()=>addCraftIngredient());['craftName','craftSpawn','craftImage'].forEach(id=>$('#'+id)?.addEventListener('input',updateCraftPreview));$('#copyCraftBtn')?.addEventListener('click',async()=>{const c=getCraftForm();try{await navigator.clipboard.writeText(craftText(c));const b=$('#copyCraftBtn'),o=b.textContent;b.textContent='COPIADO ✓';setTimeout(()=>b.textContent=o,1200)}catch{}});
$('#craftForm')?.addEventListener('submit',async e=>{e.preventDefault();if(String(currentProfile?.role||'').toUpperCase()!=='ADMIN')return;const data=getCraftForm();if(!data.nome||!data.spawn)return alert('Informe nome e spawn do produto.');const id=$('#craftId').value||craftKey(data.nome);try{await setDoc(doc(db,'highos','data','crafts',id),{...data,updatedAt:serverTimestamp(),updatedBy:currentUser.email},{merge:true});await addDoc(histCol,{tipo:'CRAFT',descricao:`Receita ${data.nome} atualizada`,craftId:id,usuario:currentUser.email,data:serverTimestamp()});$('#craftModal').classList.add('hidden');await loadCrafts()}catch(err){alert('Erro ao salvar receita: '+err.message)}});
...
// ===== HIGH OS V6.3 · ALVESINHO COM FERRAMENTAS DE CRAFT =====
const _alvesAnswerV62=alvesAnswer;
alvesAnswer=function(question=''){
 const q=alvesNorm(question),c=craftFind(question),g=alvesFindGroup(question),o=alvesFindOrg(question);
 if(c&&(q.includes('receita')||q.includes('insumo')||q.includes('precisa')||q.includes('fazer')||q.includes('fabricar')||q.includes('craft'))){return {text:craftText(c),refs:['Crafts',c.spawn]}}
 if(q.includes('craft')&&(q.includes('usa')||q.includes('usam')||q.includes('ingrediente'))){const all=craftMerged(),hit=all.filter(c=>(c.ingredientes||[]).some(i=>q.includes(alvesNorm(i.nome))||q.includes(alvesNorm(i.spawn))));if(hit.length)return {text:`Receitas encontradas:\n${hit.map(x=>`• ${x.nome} (${x.spawn})`).join('\n')}`,refs:['Crafts']}}
 if((g||o)&&(q.includes('craft')||q.includes('fabricar')||q.includes('produz'))){const target=g||faccoes.find(f=>alvesNorm(f.faccao)===alvesNorm(o?.nome));const seg=target?.segmento||o?.segmentoAtual||'';const list=craftMerged().filter(c=>(c.status||'ATIVA')==='ATIVA'&&((c.groups||[]).some(x=>alvesNorm(x)===alvesNorm(target?.group))||(!(c.groups||[]).length&&c.segmento===seg)||c.segmento===seg));if(list.length)return {text:`Crafts disponíveis como referência para ${target?.group||o?.nome||seg}:\n${list.map(x=>`• ${x.nome} (${x.spawn})`).join('\n')}`,refs:['Crafts',target?.group||seg]}}
 return _alvesAnswerV62(question);
};
