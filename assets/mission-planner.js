...
12]});}
  ...
  function renderMap(){
    ...
    if((m.category||'dominacao')==='dominacao'&&(m.zoneMode||(poly.length>=3?'polygon':'radius'))==='polygon'&&poly.length){
      const geom=dominationZoneGeometry(m),canClose=geom.mode==='polygon',zone=canClose?L.polygon(poly.map(p=>ll(p.x,p.y)),{weight:2,fillOpacity:.055,interactive:false}).addTo(state.map):(poly.length>=2?L.polyline(poly.map(p=>ll(p.x,p.y)),{weight:2,dashArray:'6 7',opacity:.85,interactive:false}).addTo(state.map):null);if(zone){zone._mpKind='zone';state.drawn.push(zone);}
