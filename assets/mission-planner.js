...
j+1];}if(d<(Number(m.spawnRadius)||100)*2)over++;}
  ...
  function requestChangeScope(m){
    ...
    const n=JSON.parse(JSON.stringify(now||{})),b=JSON.parse(JSON.stringify(before||{})),nPoints=JSON.stringify(n.points||[]),bPoints=JSON.stringify(b.points||[]),nCenter=JSON.stringify(n.center||{}),bCenter=JSON.stringify(b.center||{}),nRadius=JSON.stringify({eventRadius:n.eventRadius,circleRadius:n.circleRadius}),bRadius=JSON.stringify({eventRadius:b.eventRadius,circleRadius:b.circleRadius});
    ['points','center','eventRadius','circleRadius'].forEach(k=>{delete n[k];delete b[k];});
    const commonSame=JSON.stringify(n)===JSON.stringify(b),spawnsOnly=nPoints!==bPoints&&nCenter===bCenter&&nRadius===bRadius&&commonSame,zoneOnly=nPoints===bPoints&&(nCenter!==bCenter||nRadius!==bRadius)&&commonSame;
    return {hasBase:true,safeOnly,safeChanged:nowSafe!==beforeSafe,spawnsOnly,zoneOnly};
  }
  function highRequestExport(){
    ...
    else if(change.zoneOnly&&!creating)lines.push('','ESCOPO DA ALTERAÇÃO:','- Alteração exclusiva da área/zona deste evento.','- Manter spawns, regras, premiações e demais configurações atuais sem alteração.','- Atualizar somente o centro e o raio/zona conforme abaixo.','','ZONA ATUALIZADA:','- Centro: '+f(m.center?.x)+', '+f(m.center?.y)+', '+f(m.center?.z),'- Raio: '+Math.round(effectiveEventRadius(m))+'m');
    else{lines.push('','CONFIGURAÇÃO DA ZONA:','- Centro: '+f(m.center?.x)+', '+f(m.center?.y)+', '+f(m.center?.z),'- Raio inicial: '+Math.round(effectiveEventRadius(m))+'m','- Spawns validados: '+pts.length);pts.forEach((p,i)=>lines.push('  '+String(i+1).padStart(2,'0')+' - '+f(p.x)+', '+f(p.y)+', '+f(p.z)+', '+f(p.h)));}
