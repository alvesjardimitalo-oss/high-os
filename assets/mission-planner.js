...
j+1];}if(d<(Number(m.spawnRadius)||100)*2)over++;}
  ...
  function highRequestExport(){
    const m=active();if(!m)return '';const a=plannerAudit(m),pts=(m.points||[]).filter(isValidated),r=(m.category||'dominacao')==='gas'?ensureSafeRoute(m):null;
    const lines=['ASSUNTO:','- '+(m.requestKind==='create-event'?'Criação':'Atualização')+' do Evento '+(m.event||'Sem nome')+' - '+(m.panel||'/ilegal')+';','','SOLICITAÇÃO:','- '+(m.requestKind==='create-event'?'Solicitamos a criação':'Solicitamos a atualização')+' do evento "'+(m.event||'')+'" no painel '+(m.panel||'/ilegal')+'.','- Zona: '+(m.name||'Zona Principal')+'.','','CONFIGURAÇÃO DA ZONA:','- Centro: vec3('+f(m.center?.x)+', '+f(m.center?.y)+', '+f(m.center?.z)+')','- Raio inicial: '+Math.round(effectiveEventRadius(m))+'m','- Spawns validados: '+pts.length];
    pts.forEach((p,i)=>lines.push('  '+String(i+1).padStart(2,'0')+' - vec4('+f(p.x)+', '+f(p.y)+', '+f(p.z)+', '+f(p.h)+')'));
    if(r){lines.push('','SAFE DINÂMICA:','- Referência vertical da Safe: Z = 0.','- Fluxo: FECHA 1 → MOVE → FECHA 2 → MOVE → SAFE FINAL.');r.stages.forEach((st,i)=>lines.push('- Safe '+(i+1)+': vec3('+f(st.x)+', '+f(st.y)+', 0.00) | raio '+Math.round(Number(st.radius)||0)+'m | fechamento '+(Number(st.closeSeconds)||0)+'s'+(i<2?' | movimento '+(Number(st.moveSeconds)||0)+'s':'')));lines.push('- Duração configurada da progressão: '+safeTimeline(m).at(-1).at+' segundos.');}
    ...
    lines.push('','OBSERVAÇÕES:','- Preservar as mecânicas/regras já existentes do evento base quando aplicável.','- Spawns utilizam Z real; somente a referência visual da Safe utiliza Z = 0.');
    return lines.join('\n');
  }
