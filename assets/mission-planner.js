...
panel:m.panel||'/ilegal'});
 ...
 evt=String(m.event||'').trim();
  ...
  function dominationPolygonAudit(m){
    ...
    if(pendingZ.length)warns.push('Vértice(s) ainda pendente(s) de validação real no FiveM: '+pendingZ.map(x=>String(x.i+1).padStart(2,'0')).join(', ')+'. A geometria 2D pode ser analisada, mas confirme as CDS antes da implementação.');
...
s=coverageStats(m);if(!m||!s)return;applyRadius(s.recommended);});
  ...
  function renderDominationPolygonUi(){
    ...
    ['mpDomPolygonAdd','mpDomPolygonBulkAdd','mpDomPolygonValidateAll','mpDomPolygonUndo','mpDomPolygonClear','mpDomProposalApply'].forEach(id=>{const b=qs('#'+id);if(b)b.disabled=!state.editing;});const prop=state.zoneProposal;if(prop)el.innerHTML+='<br><br><b>Proposta visual:</b> '+esc(prop.shape)+' • '+prop.points.length+' vértices • ainda não aplicada.';
  }
...
'mpRadiusPlus100'].forEach(id=>{const b=qs('#'+id);if(b)b.disabled=!state.editing;});
  ...
  function fitZone(){
    ...
    if((m.category||'dominacao')==='dominacao'&&dominationZoneMode(m)==='polygon'&&poly.length>=3){const arr=poly.map(p=>ll(p.x,p.y));(m.points||[]).filter(isValidated).forEach(p=>arr.push(ll(p.x,p.y)));state.map.fitBounds(L.latLngBounds(arr).pad(.08),{padding:[35,35],maxZoom:6});return;}
