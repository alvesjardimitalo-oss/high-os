...
panel:m.panel||'/ilegal'});
 ...
 evt=String(m.event||'').trim();
  ...
  function dominationPolygonAudit(m){
    const raw=Array.isArray(m?.zonePolygon)?m.zonePolygon:[],indexed=raw.map((v,i)=>({v,i})).filter(x=>validCoord(x.v?.x)&&validCoord(x.v?.y)),p=indexed.map(x=>x.v),issues=[],warns=[];if(!raw.length)return {issues,warns,metrics:null};
