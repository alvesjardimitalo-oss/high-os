...
...keep];}state.editBackup=null;state.editing=false;state.dirty=false;state.placing=false;state.polygonPlacement=false;state.undoStack=[];state.redoStack=[];state.lastEditSnapshot=null;state.activeEventId=active()?.eventId||state.activeEventId;render();updateEditUi();setSaveState('Alterações descartadas • visualização');}
...
img=qs('#mpSnapshotPreview');if(!m||!img)return;const s=await getSnapshot(m.id);if(s?.dataUrl){img.src=s.dataUrl;img.classList.add('show');if(qs('#mpSnapshotEmpty'))qs('#mpSnapshotEmpty').style.display='none';}else{img.removeAttribute('src');img.classList.remove('show');if(qs('#mpSnapshotEmpty'))qs('#mpSnapshotEmpty').style.display='block';}}
  ...
  function fit(){const m=active();if(!state.map||!m)return;const arr=(m.points||[]).filter(p=>validCoord(p.x)&&validCoord(p.y)).map(p=>ll(p.x,p.y)),poly=activeDominationPolygon(m);poly.forEach(p=>arr.push(ll(p.x,p.y)));if(validCoord(m.center?.x)&&validCoord(m.center?.y))arr.push(ll(m.center.x,m.center.y));if(arr.length===1)state.map.setView(arr[0],5);else if(arr.length)state.map.fitBounds(L.latLngBounds(arr).pad(.12),{padding:[30,30],maxZoom:6});}
