...
panel:m.panel||'/ilegal'});
 ...
 evt=String(m.event||'').trim();
  ...
  function dominationZoneGeometry(m){const p=dominationPolygon(m),mode=m?.zoneMode||(p.length>=3?'polygon':'radius');if(mode==='polygon'&&p.length<3)return {mode:'polygon-incomplete',vertices:p.length,area:null,perimeter:null};if(mode!=='polygon')return {mode:'radius',vertices:0,area:Math.PI*Math.pow(effectiveEventRadius(m),2),perimeter:2*Math.PI*effectiveEventRadius(m)};return {mode:'polygon',vertices:p.length,area:polygonArea(p),perimeter:polygonPerimeter(p)};}
  ...
  function dominationPolygonAudit(m){
    ...
    if(invalid.length){warns.push('Auditoria geométrica suspensa até corrigir todas as CDS com X/Y inválidos. O sistema não ligará automaticamente os vértices vizinhos através de uma CDS inválida.');return {issues,warns,metrics:{area:null,perimeter:null,minSide:null,maxSide:null,compactness:null,crossings:null,invalidVertices:invalid.length,pendingZ:pendingZ.length,nearDuplicates:null,geometryDeferred:true}};}
    ...
    if(cross.length){const edgeLabel=k=>{const a=indexed[k-1]?.i+1,b=indexed[k%p.length]?.i+1;return String(a).padStart(2,'0')+'→'+String(b).padStart(2,'0');},detail=cross.slice(0,6).map(x=>edgeLabel(x[0])+' × '+edgeLabel(x[1])).join(', ');issues.push('O contorno da zona possui arestas que se cruzam, encostam ou se sobrepõem indevidamente'+(detail?': '+detail:'')+(cross.length>6?' (+'+(cross.length-6)+' conflito(s))':'')+'. Revise a ordem/posição das CDS antes de implementar.');}
...
j+1];}if(d<(Number(m.spawnRadius)||100)*2)over++;}
  ...
  function dominationAccessDistance(m,p){if(dominationZoneGeometry(m).mode==='polygon-incomplete')return null;if(spawnInsideDomination(m,p))return 0;const edge=polygonEdgeAccess(m,p);return edge?edge.distance:Math.max(0,pointDistanceFromCenter(m,p)-effectiveEventRadius(m));}
  ...
  function dominationAccessAnalysis(m){
    if(dominationZoneGeometry(m).mode==='polygon-incomplete')return null;
  ...
  function highRequestExport(){
    const m=active();if(!m)return '';const a=plannerAudit(m),pts=(m.points||[]).filter(isValidated),r=(m.category||'dominacao')==='gas'?ensureSafeRoute(m):null,isSurvival=/sobreviv[eê]ncia/i.test(m.event||''),change=requestChangeScope(m),polyAudit=(m.category||'dominacao')==='dominacao'?dominationPolygonAudit(m):null,polyPending=Number(polyAudit?.metrics?.pendingZ)||0;
    ...
    if(dominationZoneMode(m)==='polygon'&&polyPending>0)lines.push('','STATUS DA ENTREGA:','- RASCUNHO — NÃO IMPLEMENTAR AINDA.','- Existem '+polyPending+' vértice(s) do polígono pendente(s) de validação com CDS reais no FiveM.','- As coordenadas abaixo podem ser usadas para planejamento visual, mas devem ser substituídas/confirmadas antes da implementação.');
