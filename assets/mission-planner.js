...
j+1];}if(d<(Number(m.spawnRadius)||100)*2)over++;}
  ...
  function requestChangeScope(m){
    ...
    return {hasBase:true,safeOnly:nowSafe!==beforeSafe&&JSON.stringify(now)===JSON.stringify(before),safeChanged:nowSafe!==beforeSafe};
  }
  function highRequestExport(){
    ...
    if(change.safeOnly&&m.requestKind!=='create-event'&&m.requestKind!=='create-zone')lines.push('','ESCOPO DA ALTERAÇÃO:','- Alteração exclusiva da SAFE progressiva desta zona.','- Manter centro, raio inicial, spawns, regras e demais configurações atuais sem alteração.','- Implementar somente as etapas, movimentos e possibilidades da SAFE descritas abaixo.');
