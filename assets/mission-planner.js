...
s=coverageStats(m);if(!m||!s)return;applyRadius(s.recommended);});
  ...
  function applyZoneProposal(){if(!requireEdit())return;const m=active(),p=state.zoneProposal;if(!m||!p?.points?.length)return;m.zonePolygon=p.points.map(v=>({x:v.x,y:v.y,z:0,status:'planned',validatedAt:null,validationReason:'assisted-proposal'}));state.zoneProposal=null;commit('Proposta assistida aplicada à Zona de Dominação');}
  ...
  function ensureDominationPolygonUi(){
    ...
    box.innerHTML=`<h3>CDS DA ZONA — POLÍGONO</h3><p class="mp-note">Opcional. Cole 3 ou mais CDS na ordem do contorno. O mapa une os pontos e fecha a Zona de Pontuação automaticamente. Estas CDS não são spawns.</p><div id="mpDomPolygonStatus" class="mp-readout">Nenhum vértice cadastrado.</div><div id="mpDomPolygonList" style="margin-top:8px"></div><label style="margin-top:8px">CDS do vértice<input id="mpDomPolygonCds" type="text" placeholder="x, y, z ou vec3(x, y, z)"></label><label style="margin-top:8px">COLAR VÁRIAS CDS<textarea id="mpDomPolygonBulk" rows="5" placeholder="01 - x, y, z&#10;02 - x, y, z&#10;03 - vec3(x, y, z)"></textarea></label><div class="mp-actions" style="display:flex;gap:6px;flex-wrap:wrap"><button type="button" id="mpDomPolygonAdd">ADICIONAR VÉRTICE</button><button type="button" id="mpDomPolygonBulkAdd">IMPORTAR CDS</button><button type="button" id="mpDomPolygonUndo">REMOVER ÚLTIMO</button><button type="button" id="mpDomPolygonClear">LIMPAR POLÍGONO</button></div><div class="mp-note" style="margin-top:10px"><b>PLANEJADOR ASSISTIDO</b> • gera apenas uma proposta visual; nada é alterado até clicar APLICAR. Ao aplicar, as CDS ficam PENDENTES até validação real no FiveM.</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:8px 0"><label>ESCALA<input id="mpDomProposalScale" type="number" min="0.2" max="3" step="0.1" value="1"></label><label>ROTAÇÃO °<input id="mpDomProposalRotation" type="number" min="-180" max="180" step="5" value="0"></label></div><div class="mp-actions" style="display:flex;gap:6px;flex-wrap:wrap"><button type="button" data-zoneproposal="square">QUADRADO</button><button type="button" data-zoneproposal="rectangle">RETÂNGULO</button><button type="button" data-zoneproposal="triangle">TRIÂNGULO</button><button type="button" id="mpDomProposalApply">APLICAR PROPOSTA</button><button type="button" id="mpDomProposalCancel">CANCELAR</button></div>`;
    anchor.insertAdjacentElement('afterend',box);
    ...
    qs('#mpDomPolygonBulkAdd')?.addEventListener('click',()=>{if(!requireEdit())return;const m=active(),input=qs('#mpDomPolygonBulk');if(!m||m.category!=='dominacao'||!input)return;const lines=String(input.value||'').split(/\n+/).map(x=>x.trim()).filter(Boolean),parsed=[],bad=[];lines.forEach((line,i)=>{const clean=line.replace(/^\s*\d+\s*[-–—:.)]\s*/,'').trim(),r=parseCds(clean);if(!r.ok||!validCoord(r.x)||!validCoord(r.y)){bad.push(i+1);return;}parsed.push({x:r.x,y:r.y,z:Number.isFinite(r.z)?r.z:0,status:Number.isFinite(r.z)&&r.z!==0?'validated':'planned',validatedAt:Number.isFinite(r.z)&&r.z!==0?nowIso():null});});if(!parsed.length){alert('Nenhuma CDS válida encontrada.');return;}if(bad.length&&!confirm(parsed.length+' CDS reconhecida(s). Linha(s) não reconhecida(s): '+bad.join(', ')+'.\n\nImportar somente as CDS válidas?'))return;if(!Array.isArray(m.zonePolygon))m.zonePolygon=[];m.zonePolygon.push(...parsed);input.value='';commit(parsed.length+' CDS importadas para a Zona de Dominação na ordem informada');fitZone();});
...
j+1];}if(d<(Number(m.spawnRadius)||100)*2)over++;}
  ...
  function polygonEdgeAccess(m,p){const poly=dominationPolygon(m);if(poly.length<3)return null;let best={distance:Infinity,edge:0};for(let i=0;i<poly.length;i++){const d=pointSegmentDistance(p,poly[i],poly[(i+1)%poly.length]);if(d<best.distance)best={distance:d,edge:i+1};}return best;}
  function pointInPolygon(p,poly){if(!p||!poly||poly.length<3)return false;const x=Number(p.x),y=Number(p.y);let inside=false;for(let i=0,j=poly.length-1;i<poly.length;j=i++){const xi=Number(poly[i].x),yi=Number(poly[i].y),xj=Number(poly[j].x),yj=Number(poly[j].y);if(((yi>y)!==(yj>y))&&(x<(xj-xi)*(y-yi)/(yj-yi)+xi))inside=!inside;}return inside;}
  function spawnInsideDomination(m,p){const poly=dominationPolygon(m);if(poly.length>=3)return pointInPolygon(p,poly);if(!validCoord(m?.center?.x)||!validCoord(m?.center?.y))return false;return pointDistanceFromCenter(m,p)<=effectiveEventRadius(m);}
  function dominationAccessDistance(m,p){if(spawnInsideDomination(m,p))return 0;const edge=polygonEdgeAccess(m,p);return edge?edge.distance:Math.max(0,pointDistanceFromCenter(m,p)-effectiveEventRadius(m));}
  function nearestPolygonEdgePoint(m,p){const poly=dominationPolygon(m);if(poly.length<3)return null;let best={distance:Infinity,x:null,y:null,edge:null};for(let i=0;i<poly.length;i++){const a=poly[i],b=poly[(i+1)%poly.length],px=Number(p.x),py=Number(p.y),ax=Number(a.x),ay=Number(a.y),dx=Number(b.x)-ax,dy=Number(b.y)-ay,l2=dx*dx+dy*dy,t=l2?Math.max(0,Math.min(1,((px-ax)*dx+(py-ay)*dy)/l2)):0,x=ax+t*dx,y=ay+t*dy,d=Math.hypot(px-x,py-y);if(d<best.distance)best={distance:d,x,y,edge:i+1};}return best;}
  function dominationAccessTarget(m,p){if(spawnInsideDomination(m,p))return {x:Number(p.x),y:Number(p.y),distance:0,edge:null,inside:true};const edge=nearestPolygonEdgePoint(m,p);if(edge)return {...edge,inside:false};if(!validCoord(m?.center?.x)||!validCoord(m?.center?.y))return null;const dx=Number(p.x)-Number(m.center.x),dy=Number(p.y)-Number(m.center.y),d=Math.hypot(dx,dy),r=effectiveEventRadius(m);if(!d)return {x:Number(m.center.x),y:Number(m.center.y),distance:0,edge:null};return {x:Number(m.center.x)+dx/d*r,y:Number(m.center.y)+dy/d*r,distance:Math.max(0,d-r),edge:null};}
  function drawDominationAccess(m){if((m?.category||'dominacao')!=='dominacao'||state.layerVisibility?.access===false)return;(m.points||[]).filter(isValidated).forEach(p=>{const t=dominationAccessTarget(m,p);if(!t)return;const line=L.polyline([ll(p.x,p.y),ll(t.x,t.y)],{weight:2,opacity:.72,dashArray:'6 7',interactive:false}).addTo(state.map);line._mpKind='access';state.drawn.push(line);const label=L.marker(ll(t.x,t.y),{interactive:false,icon:L.divIcon({className:'',html:'<div style="white-space:nowrap;background:rgba(8,10,18,.82);border:1px solid rgba(255,255,255,.25);border-radius:8px;padding:2px 5px;color:#fff;font:10px system-ui">'+(t.inside?'DENTRO DA ZONA':Math.round(t.distance)+'m'+(t.edge?' • L'+t.edge:''))+'</div>',iconAnchor:[18,-4]})}).addTo(state.map);label._mpKind='access';state.drawn.push(label);});}
  ...
  function dominationAccessAnalysis(m){
    ...
    const poly=dominationPolygon(m),access=pts.map(p=>{let deg=Math.atan2(Number(p.y)-Number(center.y),Number(p.x)-Number(center.x))*180/Math.PI;if(deg<0)deg+=360;const inside=spawnInsideDomination(m,p),edge=!inside&&poly.length>=3?polygonEdgeAccess(m,p):null;return {id:p.id,deg,distanceToEdge:inside?0:(edge?edge.distance:Math.max(0,pointDistanceFromCenter(m,p)-effectiveEventRadius(m))),edge:edge?.edge||null,inside};}).sort((a,b)=>a.deg-b.deg);
  ...
  function dominationFairnessAnalysis(m){
    ...
    return {median:med,q1,q3,spread,short,long,maxEdge,total:aa.access.length,inside,external,access:aa.access};
  }
  function dominationFairnessWarnings(m){
    const f=dominationFairnessAnalysis(m),w=[];if(!f)return w;if(f.inside?.length)w.push('Spawn(s) já dentro da Zona de Pontuação: '+f.inside.map(x=>String(x.id).padStart(2,'0')).join(', ')+'. Confira se isso é intencional.');if(f.total<3)return w;
  ...
  function requestChangeScope(m){
    ...
    const commonSame=JSON.stringify(n)===JSON.stringify(b),zoneChanged=nCenter!==bCenter||nRadius!==bRadius||nPoly!==bPoly,spawnsOnly=nPoints!==bPoints&&!zoneChanged&&commonSame,zoneOnly=nPoints===bPoints&&zoneChanged&&commonSame;
    return {hasBase:true,safeOnly,safeChanged:nowSafe!==beforeSafe,spawnsOnly,zoneOnly,polygonChanged:nPoly!==bPoly,zoneChanged};
  }
  function highRequestExport(){
    ...
    else if(change.zoneOnly&&!creating){const poly=dominationPolygon(m);lines.push('','ESCOPO DA ALTERAÇÃO:','- Alteração exclusiva da Zona de Pontuação deste evento.','- Manter spawns, mecânica de pontuação, regras, premiações e demais configurações atuais sem alteração.');if(poly.length>=3){lines.push('- Atualizar os limites da Zona de Dominação pelas CDS abaixo.','','ZONA ATUALIZADA — POLÍGONO POR CDS:');poly.forEach((p,i)=>lines.push('  '+String(i+1).padStart(2,'0')+' - '+f(p.x)+', '+f(p.y)+', '+f(p.z)));lines.push('- As CDS acima formam o limite da Zona de Dominação, devem ser ligadas na ordem informada e fechadas do último ponto ao primeiro.','- Estas CDS NÃO são pontos de spawn.');}else lines.push('- Atualizar somente o centro e o raio conforme abaixo.','','ZONA ATUALIZADA — CENTRO + RAIO:','- Centro: '+f(m.center?.x)+', '+f(m.center?.y)+', '+f(m.center?.z),'- Raio: '+Math.round(effectiveEventRadius(m))+'m');}
    else{const poly=dominationPolygon(m);lines.push('','CONFIGURAÇÃO DA ZONA:');if((m.category||'dominacao')==='dominacao'&&poly.length>=3){lines.push('- Forma da Zona de Pontuação: POLÍGONO POR CDS.','- CDS que formam o limite da Zona de Dominação:');poly.forEach((p,i)=>lines.push('  '+String(i+1).padStart(2,'0')+' - '+f(p.x)+', '+f(p.y)+', '+f(p.z)));lines.push('- Ligar as CDS na ordem informada e fechar do último ponto ao primeiro.','- Estas CDS formam a zona e NÃO são spawns.');}else lines.push('- Forma da zona: CENTRO + RAIO.','- Centro: '+f(m.center?.x)+', '+f(m.center?.y)+', '+f(m.center?.z),'- Raio inicial: '+Math.round(effectiveEventRadius(m))+'m');lines.push('','SPAWNS / PONTOS DE ENTRADA:','- Spawns validados: '+pts.length);pts.forEach((p,i)=>lines.push('  '+String(i+1).padStart(2,'0')+' - '+f(p.x)+', '+f(p.y)+', '+f(p.z)+', '+f(p.h)));}
